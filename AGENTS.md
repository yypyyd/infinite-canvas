...
- 账号业务数据和账号偏好不得在浏览器持久化；禁止使用 LocalForage、IndexedDB、localStorage 或离线 outbox 保存画布、素材、生成记录、媒体和用户偏好。`localStorage` 只允许保存公告已读等非账号、极小型状态。
- 页面头部已展示的全局状态指标（如账户余额、算力），内容区不要再重复展示大数字；内容区只保留标题、说明和操作入口，指标可做成可点击跳转到对应 Tab。
...
- 画布内包含输入框、滚动条或媒体控制器的交互区域必须在根节点标记 `data-canvas-no-zoom`，确保鼠标滚轮和指针操作不会被画布缩放、平移逻辑拦截。
...
- 登录账号的画布、“我的素材”、生成记录、媒体和用户偏好只保存在服务端；浏览器仅保留当前页面生命周期内的内存状态，不提供离线保存。
- 画布生成的图片和视频只在个人中心“生成记录”中聚合展示，不得混入商品图生成或营销视频页面的工作台记录；这两个页面只展示各自页面产生的记录。
- 用户端 AI 请求只能通过同源后端 `/api/v1/*`；上游 Base URL 和 API Key 只保存在服务端私有渠道配置中，不得写入浏览器持久化。
- 用户对外 API Key 必须只保存不可逆摘要、绑定创建用户和当前企业、仅允许访问 `/api/v1/*` AI 路由；完整 Key 只在创建时展示一次，不得进入浏览器持久化、日志或审计详情。
- 初始化服务端用户偏好监听时必须先记录当前配置快照；公开模型配置、加载状态和部署后页面刷新不得被误判为用户修改并把默认值覆盖回服务端。
- 普通异步视频任务必须由服务端主动查询原渠道并归档结果，不得依赖浏览器持续轮询；只有作品成功归档、后续仍可交付时才能记成功并保留扣费，失败或最长等待窗口内仍不可交付时按创建时固化的扣费来源幂等退款，退款前必须由任务终态和账本共同防止重复返还；独立批量 Worker 管理的生成任务不得被普通任务超时扫描提前结束。
