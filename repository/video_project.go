...
func SaveVideoProject(item model.VideoProject, expectedVersion int64, storageKeys []string, auditLogs ...model.OrganizationAuditLog) (model.VideoProject, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if err := replaceUserFileReferences(tx, item.OrganizationID, "video_project_draft", item.ID, "video-project-draft-"+item.ID, storageKeys, false, item.UpdatedAt); err != nil {
			return err
		}
...
func ListVideoProjectVersions(organizationID, projectID string) ([]model.VideoProjectVersion, error) {
	...
	for index := range items {
		if err := json.Unmarshal([]byte(items[index].TimelineJSON), &items[index].Timeline); err != nil {
			return nil, err
		}
		if err := json.Unmarshal([]byte(items[index].OutputSpecJSON), &items[index].OutputSpec); err != nil {
			return nil, err
		}
	}
	return items, nil
...
func GetVideoProjectVersion(organizationID, projectID string, versionNumber int) (model.VideoProjectVersion, bool, error) {
	db, err := DB()
	if err != nil {
		return model.VideoProjectVersion{}, false, err
	}
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return model.VideoProjectVersion{}, false, nil
	}
	if err != nil {
		return item, false, err
	}
	if err := json.Unmarshal([]byte(item.TimelineJSON), &item.Timeline); err != nil {
		return item, false, err
	}
	if err := json.Unmarshal([]byte(item.OutputSpecJSON), &item.OutputSpec); err != nil {
		return item, false, err
	}
...
func CreateVideoProjectVersion(project model.VideoProject, expectedTimelineJSON string, version model.VideoProjectVersion, storageKeys []string, auditLogs ...model.OrganizationAuditLog) (model.VideoProjectVersion, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		var saved model.VideoProject
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", project.OrganizationID, project.ID).First(&saved).Error; err != nil {
			return err
		}
		if saved.Version != project.Version || saved.DraftTimelineJSON != expectedTimelineJSON {
			return ErrCommerceVersionConflict
		}
		version.Version = saved.CurrentVersion + 1
		version.TimelineJSON = saved.DraftTimelineJSON
		var timeline model.VideoTimeline
		if err := json.Unmarshal([]byte(saved.DraftTimelineJSON), &timeline); err != nil {
			return err
		}
		...
		if err := replaceUserFileReferences(tx, project.OrganizationID, "video_project_version", version.ID, "video-project-version-"+version.ID, storageKeys, false, version.CreatedAt); err != nil {
			return err
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
	})
