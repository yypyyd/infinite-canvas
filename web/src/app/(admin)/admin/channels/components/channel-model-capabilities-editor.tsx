                        ...
                        {fields.map((field) => {
                            ...
                            return (
                                <Card
                                    key={field.key}
                                    size="small"
                                    title={<Flex align="center" gap={8} wrap><Tag bordered={false}>对外模型</Tag><Typography.Text strong>{managedModel?.name || modelName}</Typography.Text>{managedModel?.name && managedModel.name !== modelName ? <Typography.Text type="secondary">{modelName}</Typography.Text> : null}</Flex>}
                                    extra={<Button type="text" danger size="small" icon={<DeleteOutlined />} onClick={() => remove(field.name)}>移除</Button>}
                                >
                                    <Form.Item name={[field.name, "model"]} hidden><Input /></Form.Item>
                                    <Row gutter={12}>
                                        <Col xs={24} md={12}>
                                        ...
                                        {canConfigureResolution ? (
                                            <Col span={24}>
                                                <Form.Item name={[field.name, "resolutionTiers"]} label="支持分辨率/清晰度档" rules={supportsResolution ? [{ required: true, message: "请选择至少一个分辨率档" }] : []} extra="图片渠道可配置 1K/2K/4K，视频渠道可配置 480P/720P/1080P；请求只会进入支持对应档位的渠道。文本、音频等无分辨率模型可留空。">
