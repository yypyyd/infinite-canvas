...
func archiveBatchProductionResult(ctx context.Context, item model.BatchProductionItem, job model.BatchProductionJob, result BatchProductionResult) (string, error) {
	...
	if existing, exists, err := repository.GetUserFile(job.OrganizationID, storageKey); err != nil {
		...
		if statErr == nil {
			if info.Size != result.Size || assetTypeFromMime(info.MimeType) != assetType {
		...
		} else if isStorageObjectNotFound(statErr) || storageObjectHTTPStatus(statErr) == 612 {
			replaceExisting = true
		} else {
	...
	info, err := putUserStorageObject(ctx, storageSetting, reservation.StorageDriver, reservation.ObjectKey, result.MimeType, job.CreatedBy, result.Size, body)
	if err != nil {
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
			return "", err
		}
	...
	file, err := repository.ConfirmUserFileUpload(job.OrganizationID, job.CreatedBy, uploadID, newID("file"), info.Hash, info.MimeType, info.Size, userStorageQuotaBytes(), now())
	if err != nil {
		return "", transientBatchProductionError(model.BatchProductionErrorStorageArchive, err)
	}
	confirmed = true
...
func batchProductionExecution(item model.BatchProductionItem, job model.BatchProductionJob) (BatchProductionExecution, error) {
	...
	if len(storageKeys) > 0 {
		...
		for _, storageKey := range storageKeys {
			...
			fileURL, ok := signedOrganizationFileURL(file, "", 30*time.Minute)
			if !ok {
				return execution, permanentBatchProductionError(model.BatchProductionErrorValidationInput, errors.New("batch input media URL is unavailable"))
			}
			execution.MediaURLs[storageKey] = fileURL
		}
	}
