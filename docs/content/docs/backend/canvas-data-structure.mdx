---
title: 画布数据结构
description: 账号画布、浏览器缓存、节点结构与媒体文件机制
---

# 画布数据结构

本文档说明账号画布的数据结构、服务端主存储、浏览器缓存和媒体文件机制。
...
- 浏览器 `localForage`：仅保存登录账号缓存、媒体缓存、离线 `workspace_outbox` 和 `workspace_meta` 上传状态。
...
4. 把 Blob 写入登录账号的本地缓存，并优先直传账号云端。
5. 离线或上传暂时失败时，由引用该文件的 outbox 在联网后补传。
6. 创建 `blob:` URL、读取图片宽高并返回：
...
- 本地缓存没有文件时，按 `storageKey` 请求同源文件接口，后端校验当前账号后返回私有文件。
- 不再导入或迁移旧游客数据和无 `storageKey` 的旧 base64 业务数据。
