...
```json
{
  "modelChannel": {
    ...
    "models": [
      { "id": "gpt-image-2", "name": "GPT Image 2", "modality": "image", "enabled": true, "sort": 0, "aspectRatios": ["1:1", "3:2", "2:3"], "resolutionTiers": ["1k", "2k", "4k"], "remark": "" }
    ],
    "groupRatios": {
      "default": 1,
      "vip": 0.8
...
| `models` | object[] | 模型管理配置，包含模型名称、类型、启用状态、排序、宽高比和分辨率档 |
