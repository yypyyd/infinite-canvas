...
func EnsureDefaultOrganization(user model.AuthUser, organizationID string, memberID string, timestamp string, auditLogs ...model.OrganizationAuditLog) (model.Organization, model.OrganizationMember, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if result.RowsAffected != 1 {
			return ErrDefaultOrganizationAlreadyCreated
		}
		if err := assignLegacyOrganizationData(tx, user.ID, organization.ID); err != nil {
			return err
		}
	...
	if errors.Is(err, ErrDefaultOrganizationAlreadyCreated) {
		...
		if readErr != nil {
			return model.Organization{}, model.OrganizationMember{}, readErr
		}
		if ok {
...
func GetOrganizationContext(userID string) (model.Organization, model.OrganizationMember, bool, error) {
	...
	findMembership := func(organizationID string) error {
		...
		if organizationID != "" {
			tx = tx.Where("m.organization_id = ?", organizationID)
		}
...
func GetOrganization(id string) (model.Organization, bool, error) {
	db, err := DB()
	if err != nil {
		return model.Organization{}, false, err
	}
	var item model.Organization
	err = db.First(&item, "id = ?", id).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return model.Organization{}, false, nil
	}
...
func CreateOrganization(organization model.Organization, membership model.OrganizationMember, setCurrent bool, auditLogs ...model.OrganizationAuditLog) (model.Organization, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if setCurrent {
			if err := tx.Model(&model.User{}).Where("id = ?", membership.UserID).Update("organization_id", organization.ID).Error; err != nil {
				return err
			}
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
...
func SwitchUserOrganization(userID string, organizationID string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		var membership model.OrganizationMember
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("user_id = ? AND organization_id = ?", userID, organizationID).First(&membership).Error; err != nil {
			return err
		}
		var organization model.Organization
		if err := tx.Where("id = ? AND status = ?", organizationID, "active").First(&organization).Error; err != nil {
			return err
		}
		if err := tx.Model(&model.User{}).Where("id = ?", userID).Update("organization_id", organizationID).Error; err != nil {
			return err
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
	})
...
func ListOrganizationInvitations(organizationID string, timestamp string) ([]model.OrganizationInvitation, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&model.OrganizationInvitation{}).Where("organization_id = ? AND status = ? AND expires_at <= ?", organizationID, model.OrganizationInvitationPending, timestamp).Updates(map[string]any{"status": model.OrganizationInvitationExpired, "updated_at": timestamp}).Error; err != nil {
			return err
		}
...
func ListUserInvitations(email string, timestamp string) ([]model.OrganizationInvitation, error) {
	...
	var items []model.OrganizationInvitation
	err = db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&model.OrganizationInvitation{}).Where("LOWER(email) = ? AND status = ? AND expires_at <= ?", strings.ToLower(email), model.OrganizationInvitationPending, timestamp).Updates(map[string]any{"status": model.OrganizationInvitationExpired, "updated_at": timestamp}).Error; err != nil {
			return err
		}
...
func SaveOrganizationInvitation(invitation model.OrganizationInvitation, outbox model.OrganizationEmailOutbox, auditLogs ...model.OrganizationAuditLog) (model.OrganizationInvitation, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		var organization model.Organization
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&organization, "id = ?", invitation.OrganizationID).Error; err != nil {
			return err
		}
		...
		var total int64
		if err := tx.Model(&model.OrganizationInvitation{}).Where("organization_id = ? AND LOWER(email) = ? AND status = ?", invitation.OrganizationID, strings.ToLower(invitation.Email), model.OrganizationInvitationPending).Count(&total).Error; err != nil {
			return err
		}
		if total > 0 {
...
func ClaimOrganizationEmailOutbox(timestamp string, leaseExpiresAt string, leaseToken string) (model.OrganizationEmailOutbox, bool, error) {
	db, err := DB()
	if err != nil {
		return model.OrganizationEmailOutbox{}, false, err
	}
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.OrganizationEmailOutbox{}).Where("status IN ? AND invitation_id IN (?)", []string{"pending", "failed"}, unavailableInvitations).Updates(map[string]any{"status": "cancelled", "updated_at": timestamp}).Error; err != nil {
			return err
		}
	...
	if !found {
		return model.OrganizationEmailOutbox{}, false, nil
	}
...
func RevokeOrganizationInvitation(organizationID string, id string, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.OrganizationEmailOutbox{}).Where("organization_id = ? AND invitation_id = ? AND status IN ?", organizationID, id, []string{"pending", "failed", "processing"}).Updates(map[string]any{"status": "cancelled", "lease_token": "", "lease_expires_at": "", "updated_at": timestamp}).Error; err != nil {
			return err
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
	})
...
func GetOrganizationInvitation(id string) (model.OrganizationInvitation, bool, error) {
	db, err := DB()
	if err != nil {
		return model.OrganizationInvitation{}, false, err
	}
	var item model.OrganizationInvitation
	err = db.First(&item, "id = ?", id).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return model.OrganizationInvitation{}, false, nil
	}
	return item, err == nil, err
}
...
func AcceptOrganizationInvitation(invitation model.OrganizationInvitation, membership model.OrganizationMember, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		var organization model.Organization
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id = ? AND status = ?", invitation.OrganizationID, "active").First(&organization).Error; err != nil {
			return err
		}
		...
		if err := tx.Model(&model.OrganizationEmailOutbox{}).Where("organization_id = ? AND invitation_id = ? AND status IN ?", invitation.OrganizationID, invitation.ID, []string{"pending", "failed", "processing"}).Updates(map[string]any{"status": "cancelled", "lease_token": "", "lease_expires_at": "", "updated_at": timestamp}).Error; err != nil {
			return err
		}
		if err := tx.Create(&membership).Error; err != nil {
			return err
		}
		if err := tx.Model(&model.User{}).Where("id = ?", membership.UserID).Update("organization_id", membership.OrganizationID).Error; err != nil {
			return err
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
	})
...
func ListOrganizationAuditLogs(organizationID string, q model.Query) ([]model.OrganizationAuditLog, int64, error) {
	...
	if q.Keyword != "" {
		tx = tx.Where("action LIKE ? OR resource_type LIKE ? OR detail LIKE ?", "%"+q.Keyword+"%", "%"+q.Keyword+"%", "%"+q.Keyword+"%")
	}
	var total int64
...
func SaveOrganization(organization model.Organization, auditLogs ...model.OrganizationAuditLog) (model.Organization, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&saved, "id = ?", organization.ID).Error; err != nil {
			return err
		}
		if saved.Version != organization.Version-1 {
...
func SaveOrganizationMember(member model.OrganizationMember, auditLogs ...model.OrganizationAuditLog) (model.OrganizationMember, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		var organization model.Organization
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&organization, "id = ?", member.OrganizationID).Error; err != nil {
			return err
		}
		var saved model.OrganizationMember
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", member.OrganizationID, member.ID).First(&saved).Error; err != nil {
			return err
		}
		if saved.Role == model.OrganizationRoleOwner {
			return ErrOrganizationOwnershipChanged
		}
		if saved.Version != member.Version-1 {
			return ErrOrganizationMemberVersionConflict
		}
...
func DeleteOrganizationMember(organizationID string, memberID string, expectedVersion int64, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("organization_id = ? AND id = ?", organizationID, memberID).First(&member).Error; err != nil {
			return err
		}
		if member.Role == model.OrganizationRoleOwner {
			return ErrOrganizationOwnershipChanged
		}
		...
		if err := tx.Model(&model.User{}).Where("id = ? AND organization_id = ?", member.UserID, organizationID).Update("organization_id", "").Error; err != nil {
			return err
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
	})
...
func TransferOrganizationOwnership(organizationID string, ownerMemberID string, targetMemberID string, targetExpectedVersion int64, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("organization_id = ? AND role = ?", organizationID, model.OrganizationRoleOwner).First(&owner).Error; err != nil {
			return err
		}
		if owner.ID != ownerMemberID {
			return ErrOrganizationOwnershipChanged
		}
		var target model.OrganizationMember
		if err := tx.Where("organization_id = ? AND id = ? AND role <> ?", organizationID, targetMemberID, model.OrganizationRoleOwner).First(&target).Error; err != nil {
			return err
		}
		if target.Version != targetExpectedVersion {
			return ErrOrganizationMemberVersionConflict
		}
...
func SaveBrand(item model.Brand, create bool, auditLogs ...model.OrganizationAuditLog) (model.Brand, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if create {
			if err := tx.Create(&item).Error; err != nil {
				if isDuplicateKeyError(err) {
					return ErrBrandNameConflict
				}
				return err
			}
		} else {
			var saved model.Brand
			if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", item.OrganizationID, item.ID).First(&saved).Error; err != nil {
				return err
			}
			if saved.Version != item.Version-1 {
				return ErrCommerceVersionConflict
			}
		...
		storageKeys := []string{}
		if item.LogoStorageKey != "" {
			storageKeys = append(storageKeys, item.LogoStorageKey)
		}
		if err := replaceUserFileReferences(tx, item.OrganizationID, "brand", item.ID, "brand-"+item.ID, storageKeys, false, item.UpdatedAt); err != nil {
			return err
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
	})
...
func GetBrand(organizationID string, id string) (model.Brand, bool, error) {
	db, err := DB()
	if err != nil {
		return model.Brand{}, false, err
	}
	var item model.Brand
	err = db.Where("organization_id = ? AND id = ?", organizationID, id).First(&item).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return model.Brand{}, false, nil
	}
	return item, err == nil, err
}
...
func DeleteBrand(organizationID string, id string, expectedVersion int64, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", organizationID, id).First(&brand).Error; err != nil {
			return err
		}
		...
		if err := tx.Model(&model.Product{}).Where("organization_id = ? AND brand_id = ?", organizationID, id).Count(&references).Error; err != nil {
			return err
		}
		if references == 0 {
			if err := tx.Model(&model.BatchProductionJob{}).Where("organization_id = ? AND brand_id = ?", organizationID, id).Count(&references).Error; err != nil {
				return err
			}
		}
		if references > 0 {
		...
		if err := replaceUserFileReferences(tx, organizationID, "brand", id, "brand-"+id, nil, true, timestamp); err != nil {
			return err
		}
...
func ListProducts(organizationID string, q model.Query) ([]model.Product, int64, error) {
	...
	for _, row := range rows {
		counts[row.ProductID] = row.Total
	}
	for index := range items {
		items[index].SKUCount = counts[items[index].ID]
	}
	return items, total, nil
}
...
func SaveProduct(item model.Product, create bool, auditLogs ...model.OrganizationAuditLog) (model.Product, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		if item.BrandID != "" {
			var brand model.Brand
			if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", item.OrganizationID, item.BrandID).First(&brand).Error; err != nil {
				return err
			}
		}
		if create {
...
func DeleteProduct(organizationID string, id string, expectedVersion int64, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", organizationID, id).First(&product).Error; err != nil {
			return err
		}
		...
		var references int64
		if err := tx.Model(&model.BatchProductionItem{}).Where("organization_id = ? AND product_id = ?", organizationID, id).Count(&references).Error; err != nil {
			return err
		}
		if references > 0 {
		...
		var skuIDs []string
		if err := tx.Model(&model.ProductSKU{}).Where("organization_id = ? AND product_id = ?", organizationID, id).Pluck("id", &skuIDs).Error; err != nil {
			return err
		}
		storageKeys := []string{}
		for start := 0; start < len(skuIDs); start += 200 {
			...
			if err := tx.Model(&model.UserFileReference{}).Where("organization_id = ? AND domain = ? AND object_id IN ?", organizationID, "sku", skuIDs[start:end]).Pluck("storage_key", &batchKeys).Error; err != nil {
				return err
			}
			storageKeys = append(storageKeys, batchKeys...)
			if err := tx.Where("organization_id = ? AND domain = ? AND object_id IN ?", organizationID, "sku", skuIDs[start:end]).Delete(&model.UserFileReference{}).Error; err != nil {
				return err
			}
		}
		if err := refreshUserFileReferenceState(tx, organizationID, storageKeys, timestamp); err != nil {
			return err
		}
		if err := tx.Where("organization_id = ? AND product_id = ?", organizationID, id).Delete(&model.ProductSKU{}).Error; err != nil {
			return err
		}
...
func ListProductSKUs(organizationID string, productID string, q model.Query) ([]model.ProductSKU, int64, error) {
	...
	if q.Keyword != "" {
		tx = tx.Where("name LIKE ? OR code LIKE ?", "%"+q.Keyword+"%", "%"+q.Keyword+"%")
	}
	if q.Type != "" && q.Type != "all" {
...
func SaveProductSKU(item model.ProductSKU, create bool, auditLogs ...model.OrganizationAuditLog) (model.ProductSKU, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		var product model.Product
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", item.OrganizationID, item.ProductID).First(&product).Error; err != nil {
			return err
		}
		if create {
			if err := tx.Create(&item).Error; err != nil {
				if isDuplicateKeyError(err) {
					return ErrProductSKUCodeConflict
				}
				return err
			}
		} else {
			var saved model.ProductSKU
			if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ? AND product_id = ?", item.OrganizationID, item.ID, item.ProductID).First(&saved).Error; err != nil {
				return err
			}
			if saved.Version != item.Version-1 {
				return ErrCommerceVersionConflict
		...
		if err := replaceUserFileReferences(tx, item.OrganizationID, "sku", item.ID, "sku-"+item.ID, item.ImageStorageKeys, false, item.UpdatedAt); err != nil {
			return err
		}
...
func GetProductSKU(organizationID string, id string) (model.ProductSKU, bool, error) {
	db, err := DB()
	if err != nil {
		return model.ProductSKU{}, false, err
	}
	var item model.ProductSKU
	err = db.Where("organization_id = ? AND id = ?", organizationID, id).First(&item).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return model.ProductSKU{}, false, nil
	}
	return item, err == nil, err
}
...
func DeleteProductSKU(organizationID string, id string, expectedVersion int64, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", organizationID, id).First(&sku).Error; err != nil {
			return err
		}
		...
		var product model.Product
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", organizationID, sku.ProductID).First(&product).Error; err != nil {
			return err
		}
		var references int64
		if err := tx.Model(&model.BatchProductionItem{}).Where("organization_id = ? AND sku_id = ?", organizationID, id).Count(&references).Error; err != nil {
			return err
		}
		if references > 0 {
			return ErrProductSKUInUse
		}
		if err := replaceUserFileReferences(tx, organizationID, "sku", id, "sku-"+id, nil, true, timestamp); err != nil {
			return err
		}
...
func TransferCreditsToOrganization(organizationID string, userID string, amount int, timestamp string, personalLog model.CreditLog, organizationLog model.CreditLog, auditLogs ...model.OrganizationAuditLog) (model.Organization, model.User, error) {
	db, err := DB()
	if err != nil {
		return model.Organization{}, model.User{}, err
	}
	var organization model.Organization
	var user model.User
	err = db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id = ? AND status = ?", organizationID, "active").First(&organization).Error; err != nil {
			return err
		}
		var membership model.OrganizationMember
		if err := tx.Where("organization_id = ? AND user_id = ?", organizationID, userID).First(&membership).Error; err != nil {
			return err
		}
		...
		if err := tx.Create(&personalLog).Error; err != nil {
			return err
		}
		if err := tx.Create(&organizationLog).Error; err != nil {
			return err
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
	})
...
func GetProductionTemplate(organizationID string, templateID string) (model.ProductionTemplate, bool, error) {
	db, err := DB()
	if err != nil {
		return model.ProductionTemplate{}, false, err
	}
...
func GetProductionTemplateVersion(organizationID string, templateID string, versionNumber int) (model.ProductionTemplate, model.ProductionTemplateVersion, bool, error) {
	db, err := DB()
	if err != nil {
		return model.ProductionTemplate{}, model.ProductionTemplateVersion{}, false, err
	}
	var item model.ProductionTemplate
	if err := db.Where("organization_id = ? AND id = ?", organizationID, templateID).First(&item).Error; errors.Is(err, gorm.ErrRecordNotFound) {
		return model.ProductionTemplate{}, model.ProductionTemplateVersion{}, false, nil
	} else if err != nil {
		return model.ProductionTemplate{}, model.ProductionTemplateVersion{}, false, err
	}
	if versionNumber <= 0 {
	...
	if err := db.Where("organization_id = ? AND template_id = ? AND version = ?", organizationID, templateID, versionNumber).First(&version).Error; errors.Is(err, gorm.ErrRecordNotFound) {
		return item, model.ProductionTemplateVersion{}, false, nil
	} else if err != nil {
		return item, model.ProductionTemplateVersion{}, false, err
	}
...
func SaveProductionTemplate(item model.ProductionTemplate, expectedVersion int64, timestamp string, auditLogs ...model.OrganizationAuditLog) (model.ProductionTemplate, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if errors.Is(find.Error, gorm.ErrRecordNotFound) {
			...
			if err := tx.Create(&item).Error; err != nil {
				if isDuplicateKeyError(err) {
					return ErrProductionTemplateNameConflict
				}
				return err
			}
		} else {
...
func PublishProductionTemplate(organizationID string, templateID string, expectedVersion int64, versionID string, createdBy string, timestamp string, auditLogs ...model.OrganizationAuditLog) (model.ProductionTemplate, model.ProductionTemplateVersion, error) {
	db, err := DB()
	if err != nil {
		return model.ProductionTemplate{}, model.ProductionTemplateVersion{}, err
	}
	var item model.ProductionTemplate
	var version model.ProductionTemplateVersion
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", organizationID, templateID).First(&item).Error; err != nil {
			return err
		}
...
func CreateBatchProductionJob(job model.BatchProductionJob, itemPricing map[string]model.BatchProductionItemPricing, auditLogs ...model.OrganizationAuditLog) (model.BatchProductionJob, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if job.BrandID != "" {
			var saved model.Brand
			if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", job.OrganizationID, job.BrandID).First(&saved).Error; err != nil {
				return err
			}
			brand = &saved
		}
		var products []model.Product
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id IN ?", job.OrganizationID, job.ProductIDs).Order("id asc").Find(&products).Error; err != nil {
			return err
		}
		if len(products) != len(job.ProductIDs) {
			return gorm.ErrRecordNotFound
		}
		...
		if err := tx.Where("organization_id = ? AND product_id IN ?", job.OrganizationID, job.ProductIDs).Order("created_at asc, id asc").Limit(maxBatchProductionItems + 1).Find(&skus).Error; err != nil {
			return err
		}
		if len(skus) > maxBatchProductionItems {
		...
		for _, sku := range skus {
			covered[sku.ProductID] = true
			if err := appendItem(sku.ProductID, sku.ID); err != nil {
				return err
			}
		}
		for _, product := range products {
			if !covered[product.ID] {
				if err := appendItem(product.ID, ""); err != nil {
					return err
				}
			}
		}
		...
		brandSnapshotID := ""
		if brand != nil {
			id, snapshotErr := appendSnapshot("brand", brand.ID, *brand)
			if snapshotErr != nil {
				return snapshotErr
			}
			brandSnapshotID = id
		...
		for _, product := range products {
			...
			productSnapshotIDs[product.ID] = id
		}
		...
		for _, sku := range skus {
			...
			skuSnapshotIDs[sku.ID] = id
		}
		for index, item := range items {
			product, ok := productByID[item.ProductID]
			if !ok || item.OrganizationID != job.OrganizationID || item.JobID != job.ID {
				return gorm.ErrRecordNotFound
			}
			...
			if item.SKUID != "" {
				...
				if !ok || sku.ProductID != item.ProductID {
					return gorm.ErrRecordNotFound
				}
		...
		if brand != nil && brand.LogoStorageKey != "" {
...
func GetBatchProductionSnapshots(organizationID string, ids []string) (map[string]model.BatchProductionSnapshot, error) {
	...
	if err := db.Where("organization_id = ? AND id IN ?", organizationID, ids).Find(&items).Error; err != nil {
		return nil, err
	}
...
func aggregateBatchProductionJob(tx *gorm.DB, job *model.BatchProductionJob, timestamp string) error {
	...
	if err := tx.Model(&model.BatchProductionItem{}).
		Select(`
			COUNT(*) AS total,
			...
			COALESCE(SUM(CASE WHEN started_at <> '' THEN 1 ELSE 0 END), 0) AS started,
			COALESCE(SUM(CASE WHEN status = ? AND review_status = ? THEN 1 ELSE 0 END), 0) AS pending_reviews,
			COALESCE(SUM(CASE WHEN status = ? AND review_status = ? THEN 1 ELSE 0 END), 0) AS rejected`,
			...
			model.BatchProductionStatusCompleted,
			model.BatchProductionReviewPending,
			model.BatchProductionStatusCompleted,
			model.BatchProductionReviewRejected,
		).
	...
	if status != model.BatchProductionStatusCancelled {
		if counts.Queued > 0 || counts.Running > 0 {
			...
			if counts.Running > 0 || counts.Completed > 0 || counts.Failed > 0 || counts.Started > 0 {
			...
			} else if counts.PendingReviews > 0 {
	...
	job.Status = status
	job.TotalItems = int(counts.Total)
	job.QueuedItems = int(counts.Queued)
	job.RunningItems = int(counts.Running)
	job.CompletedItems = int(counts.Completed)
	job.FailedItems = int(counts.Failed)
...
func CancelBatchProductionJob(organizationID string, id string, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ? AND status IN ?", organizationID, id, []model.BatchProductionStatus{model.BatchProductionStatusQueued, model.BatchProductionStatusRunning}).First(&job).Error; errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrBatchProductionStateConflict
		} else if err != nil {
			return err
		}
		if err := tx.Model(&model.BatchProductionItem{}).Where("organization_id = ? AND job_id = ? AND status IN ?", organizationID, id, []model.BatchProductionStatus{model.BatchProductionStatusQueued, model.BatchProductionStatusRunning}).Updates(map[string]any{"status": model.BatchProductionStatusCancelled, "error_code": model.BatchProductionErrorCancelledLeaseLost, "retryable": false, "next_attempt_at": "", "lease_token": "", "lease_expires_at": "", "locked_at": "", "finished_at": timestamp, "updated_at": timestamp}).Error; err != nil {
			return err
		}
		job.Status = model.BatchProductionStatusCancelled
		if err := aggregateBatchProductionJob(tx, &job, timestamp); err != nil {
			return err
		}
		return saveOrganizationAuditLogs(tx, auditLogs)
	})
...
func ListBatchProductionItems(organizationID string, jobID string, q model.Query) ([]model.BatchProductionItem, int64, error) {
	...
	if q.Keyword != "" {
		tx = tx.Where("product_id LIKE ? OR sku_id LIKE ? OR error_message LIKE ? OR review_comment LIKE ?", "%"+q.Keyword+"%", "%"+q.Keyword+"%", "%"+q.Keyword+"%", "%"+q.Keyword+"%")
	}
	if q.Type != "" && q.Type != "all" {
		tx = tx.Where("status = ?", q.Type)
	}
	...
	if err := tx.Order("created_at asc").Offset(q.Offset()).Limit(q.PageSize).Find(&items).Error; err != nil {
		return nil, 0, err
	}
	storageKeys := make([]string, 0, len(items))
	for _, item := range items {
		if item.ResultStorageKey != "" {
			storageKeys = append(storageKeys, item.ResultStorageKey)
		}
	}
	...
	if err := db.Select("storage_key", "mime_type", "size").Where("organization_id = ? AND storage_key IN ?", organizationID, storageKeys).Find(&files).Error; err != nil {
		return nil, 0, err
	}
	fileByKey := make(map[string]model.UserFile, len(files))
	for _, file := range files {
		fileByKey[file.StorageKey] = file
	}
	for index := range items {
		if file, ok := fileByKey[items[index].ResultStorageKey]; ok {
			items[index].ResultMimeType, items[index].ResultSize = file.MimeType, file.Size
		}
	}
	return items, total, nil
}
...
func ListBatchProductionArchiveItems(organizationID string, jobID string) (model.BatchProductionJob, []model.BatchProductionArchiveItem, error) {
	db, err := DB()
	if err != nil {
		return model.BatchProductionJob{}, nil, err
	}
	var job model.BatchProductionJob
	if err := db.Where("organization_id = ? AND id = ?", organizationID, jobID).First(&job).Error; err != nil {
		return model.BatchProductionJob{}, nil, err
	}
...
func ReviewBatchProductionItem(organizationID string, jobID string, itemID string, runNumber int, status model.BatchProductionReviewStatus, comment string, reviewerID string, timestamp string, auditLogs ...model.OrganizationAuditLog) (model.BatchProductionItem, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		var job model.BatchProductionJob
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ?", organizationID, jobID).First(&job).Error; errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrBatchProductionStateConflict
		} else if err != nil {
			return err
		}
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND job_id = ? AND id = ? AND status = ? AND run_number = ?", organizationID, jobID, itemID, model.BatchProductionStatusCompleted, runNumber).First(&item).Error; errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrBatchProductionStateConflict
		} else if err != nil {
			return err
		}
		...
		if status == model.BatchProductionReviewRejected {
...
func SetBatchProductionPrimary(organizationID string, jobID string, itemID string, runNumber int, timestamp string, auditLogs ...model.OrganizationAuditLog) (model.BatchProductionItem, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("organization_id = ? AND job_id = ? AND id = ? AND status = ? AND run_number = ? AND review_status = ? AND result_storage_key <> ''", organizationID, jobID, itemID, model.BatchProductionStatusCompleted, runNumber, model.BatchProductionReviewApproved).First(&candidate).Error; errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrBatchProductionStateConflict
		} else if err != nil {
			return err
		}
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND product_id = ? AND sku_id = ?", organizationID, candidate.ProductID, candidate.SKUID).Order("id asc").Find(&scopeItems).Error; err != nil {
			return err
		}
		...
		if err := tx.Model(&model.BatchProductionItem{}).Where("organization_id = ? AND product_id = ? AND sku_id = ? AND id <> ? AND is_primary = ?", organizationID, item.ProductID, item.SKUID, item.ID, true).Update("is_primary", false).Error; err != nil {
			return err
		}
...
func RetryBatchProductionItem(organizationID string, jobID string, itemID string, runNumber int, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		var job model.BatchProductionJob
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ? AND status <> ?", organizationID, jobID, model.BatchProductionStatusCancelled).First(&job).Error; errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrBatchProductionStateConflict
		} else if err != nil {
			return err
		}
		var item model.BatchProductionItem
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND job_id = ? AND id = ? AND run_number = ?", organizationID, jobID, itemID, runNumber).First(&item).Error; errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrBatchProductionStateConflict
		} else if err != nil {
			return err
		}
		if item.Status != model.BatchProductionStatusFailed && (item.Status != model.BatchProductionStatusCompleted || item.ReviewStatus != model.BatchProductionReviewRejected) {
			return ErrBatchProductionStateConflict
		}
		var pending int64
		if err := tx.Model(&model.BatchProductionItem{}).Where("organization_id = ? AND status IN ?", organizationID, []model.BatchProductionStatus{model.BatchProductionStatusQueued, model.BatchProductionStatusRunning}).Count(&pending).Error; err != nil {
			return err
		}
		if pending+1 > maxBatchProductionPendingItemsPerOrganization {
			return ErrBatchProductionOrganizationQueueFull
		}
		...
		if err := replaceUserFileReferences(tx, organizationID, "batch_result", item.ID, "batch-result-"+item.ID, nil, true, timestamp); err != nil {
			return err
		}
		if err := aggregateBatchProductionJob(tx, &job, timestamp); err != nil {
			return err
...
func RetryBatchProductionJob(organizationID string, id string, timestamp string, auditLogs ...model.OrganizationAuditLog) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		var job model.BatchProductionJob
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id = ? AND status IN ?", organizationID, id, []model.BatchProductionStatus{model.BatchProductionStatusFailed, model.BatchProductionStatusPartialSuccess}).First(&job).Error; errors.Is(err, gorm.ErrRecordNotFound) {
			return ErrBatchProductionStateConflict
		} else if err != nil {
			return err
		}
		var retryItems []model.BatchProductionItem
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND job_id = ? AND status = ?", organizationID, id, model.BatchProductionStatusFailed).Order("id asc").Find(&retryItems).Error; err != nil {
			return err
		}
		if len(retryItems) == 0 {
			return ErrBatchProductionStateConflict
		...
		if pending+int64(len(retryItems)) > maxBatchProductionPendingItemsPerOrganization {
			return ErrBatchProductionOrganizationQueueFull
		}
		retryCredits := 0
		...
		if err := reserveBatchProductionRetryCredits(tx, &job, retryCredits, timestamp); err != nil {
			return err
		}
		...
		if result.RowsAffected != int64(len(retryItems)) {
			return ErrBatchProductionStateConflict
		}
		for _, item := range retryItems {
			if err := replaceUserFileReferences(tx, organizationID, "batch_result", item.ID, "batch-result-"+item.ID, nil, true, timestamp); err != nil {
...
func ClaimNextBatchProductionItem(timestamp string, leaseExpiresAt string, leaseToken string, maxTenantRunning int) (model.BatchProductionItem, model.BatchProductionJob, bool, error) {
	db, err := DB()
	if err != nil {
		return model.BatchProductionItem{}, model.BatchProductionJob{}, false, err
	}
	if maxTenantRunning < 1 {
		maxTenantRunning = 1
	}
	if err := failExhaustedBatchProductionItems(db, timestamp, 5); err != nil {
		return model.BatchProductionItem{}, model.BatchProductionJob{}, false, err
	}
	for attempt := 0; attempt < 3; attempt++ {
		...
		err = db.Transaction(func(tx *gorm.DB) error {
			...
			candidate := tx.Table("organizations").
				...
				Limit(1)
			if err := candidate.Find(&candidates).Error; err != nil {
				return err
			}
			if len(candidates) == 0 {
				return gorm.ErrRecordNotFound
			...
			if err := itemCandidate.First(&item).Error; err != nil {
				return err
			}
			if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&job, "id = ? AND organization_id = ? AND status IN ?", item.JobID, selected.OrganizationID, []model.BatchProductionStatus{model.BatchProductionStatusQueued, model.BatchProductionStatusRunning}).Error; err != nil {
				return err
			}
			var organization model.Organization
			if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Select("id, status, batch_claim_cursor").First(&organization, "id = ?", selected.OrganizationID).Error; err != nil {
				return err
			}
			if organization.Status != "active" || organization.BatchClaimCursor != selected.BatchClaimCursor {
				return nil
			}
			var activeRunning int64
			if err := tx.Model(&model.BatchProductionItem{}).Where("organization_id = ? AND status = ? AND lease_expires_at > ?", selected.OrganizationID, model.BatchProductionStatusRunning, timestamp).Count(&activeRunning).Error; err != nil {
				return err
			}
			if activeRunning >= int64(maxTenantRunning) {
				return nil
			}
			...
			if err := tx.Model(&model.Organization{}).Where("id = ?", organization.ID).Update("batch_claim_cursor", timestamp+":"+leaseToken).Error; err != nil {
				return err
			}
			claimed = true
...
func failExhaustedBatchProductionItems(db *gorm.DB, timestamp string, maxAttempts int) error {
	return db.Transaction(func(tx *gorm.DB) error {
		var items []model.BatchProductionItem
		if err := tx.Where("status = ? AND attempts >= ? AND lease_expires_at <> '' AND lease_expires_at <= ?", model.BatchProductionStatusRunning, maxAttempts, timestamp).Order("organization_id asc, job_id asc, id asc").Limit(500).Find(&items).Error; err != nil {
			return err
		}
...
func FinishBatchProductionItemWithSchedule(item model.BatchProductionItem, status model.BatchProductionStatus, resultStorageKey string, errorMessage string, errorCategory model.BatchProductionErrorCategory, retryable bool, reserveRetryCredits bool, nextAttemptAt string, timestamp string) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		var job model.BatchProductionJob
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&job, "id = ? AND organization_id = ?", item.JobID, item.OrganizationID).Error; err != nil {
			return err
		}
		if job.Status == model.BatchProductionStatusCancelled || job.Status == model.BatchProductionStatusCompleted || job.Status == model.BatchProductionStatusFailed || job.Status == model.BatchProductionStatusPendingReview || job.Status == model.BatchProductionStatusPartialSuccess {
			return ErrBatchProductionLeaseLost
		}
		if retryable {
		...
		storageKeys := []string{}
		if resultStorageKey != "" {
			storageKeys = append(storageKeys, resultStorageKey)
		}
		if err := replaceUserFileReferences(tx, item.OrganizationID, "batch_result", item.ID, "batch-result-"+item.ID, storageKeys, status != model.BatchProductionStatusCompleted, timestamp); err != nil {
			return err
		}
