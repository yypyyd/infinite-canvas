...
func TestOrganizationCreditsHTTPSettingsAndTransfer(t *testing.T) {
	...
	if organization.Credits != 30 || user.Credits != 70 { t.Fatalf("unexpected transfer balances: organization=%d user=%d", organization.Credits, user.Credits) }
	if err := database.Model(&model.Organization{}).Where("id = ?", tenant.Organization.ID).Updates(map[string]any{"monthly_credits_used": 30, "credit_budget_month": time.Now().UTC().Format("2006-01")}).Error; err != nil { t.Fatal(err) }
	...
	if workspaceResponse.Code != 0 || json.Unmarshal(workspaceResponse.Data, &workspace) != nil || !workspace.CreditSummary.Warning { t.Fatalf("credit warning response: %#v workspace=%#v", workspaceResponse, workspace) }
	var logs []model.CreditLog
	if err := database.Where("organization_id = ?", tenant.Organization.ID).Order("created_at asc, id asc").Find(&logs).Error; err != nil { t.Fatal(err) }
	...
	if response := routerTestJSON(t, ownerClient, http.MethodPost, baseURL+"/api/commerce/organization-credit-settings", map[string]any{"mode": "personal", "monthlyBudget": 0, "alertThreshold": 80, "version": 1}, map[string]string{"X-Organization-ID": tenant.Foreign.ID}); response.Code != 1 { t.Fatalf("foreign credit settings response: %#v", response) }
	...
	if err := database.Create(&memberUser).Error; err != nil { t.Fatal(err) }
