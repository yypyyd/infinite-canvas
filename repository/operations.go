...
func GetDataConsistencySnapshot() (DataConsistencySnapshot, error) {
	db, err := DB()
	if err != nil {
		return DataConsistencySnapshot{}, err
	}
	...
	queries := []*gorm.DB{
		...
		db.Select("id, organization_id, user_id, storage_key, storage_driver, object_key, mime_type, size, reserved_bytes, expires_at, created_at").Find(&snapshot.Reservations),
		db.Select("id, organization_id, storage_driver, object_key, size, status").Find(&snapshot.Deletions),
	...
	for _, query := range queries {
		if query.Error != nil {
			return DataConsistencySnapshot{}, query.Error
		}
	}
	return snapshot, nil
}
...
func RepairDanglingFileReference(id string) (bool, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		var files int64
		if err := tx.Model(&model.UserFile{}).Where("organization_id = ? AND storage_key = ?", reference.OrganizationID, reference.StorageKey).Count(&files).Error; err != nil {
			return err
		}
		if files > 0 {
			return nil
...
func RepairUserFileReferenceState(id string, timestamp string) (bool, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.UserFileReference{}).Where("organization_id = ? AND storage_key = ?", file.OrganizationID, file.StorageKey).Count(&references).Error; err != nil {
			return err
		}
		expected := timestamp
		if references > 0 {
		...
		if file.UnreferencedAt == expected || (references == 0 && file.UnreferencedAt != "") {
			return nil
		}
...
func RepairBatchProductionResultReference(id string, timestamp string) (bool, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if item.Status != model.BatchProductionStatusCompleted || item.ResultStorageKey == "" {
			return nil
		}
		var file model.UserFile
		...
		if err := tx.Model(&model.UserFileReference{}).Where("organization_id = ? AND domain = ? AND object_id = ?", item.OrganizationID, "batch_result", item.ID).Count(&references).Error; err != nil {
			return err
		}
		if err := tx.Model(&model.UserFileReference{}).Where("organization_id = ? AND domain = ? AND object_id = ? AND storage_key = ?", item.OrganizationID, "batch_result", item.ID, item.ResultStorageKey).Count(&validReferences).Error; err != nil {
			return err
		}
		if references == 1 && validReferences == 1 {
			return nil
		}
		if err := replaceUserFileReferences(tx, item.OrganizationID, "batch_result", item.ID, "batch-result-"+item.ID, []string{item.ResultStorageKey}, false, timestamp); err != nil {
			return err
		}
		repaired = true
		return nil
