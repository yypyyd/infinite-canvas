...
export default function CommercePage() {
    ...
    const saveBatchItemToCanvas = async (item: BatchProductionItem) => {
        ...
        try {
            ...
            const projectId = existing?.id || store.importProject({
                title: `${selectedBatch?.name || "批量结果"} · ${item.skuId || item.productId}`,
                nodes: [{ id: nodeId, type: CanvasNodeType.Image, title: item.isPrimary ? "商品主图" : "批量生产结果", position: { x: 80, y: 80 }, width: 320, height: 320, metadata: { status: "success", storageKey: item.resultStorageKey, mimeType: "image/*" } }],
            });
            await flushActiveWorkspaceChanges();
    ...
    const previewBatchPrompt = async () => {
        ...
        if (!productId) { message.warning("请先选择至少一个商品"); return; }
        setWorking(true);
        try {
            const result = await previewProductionPrompt({ presetId: value.presetId, presetVersion: batchForm.getFieldValue("presetVersion") || 1, deliverySpecId: batchForm.getFieldValue("deliverySpecId") || "original", brandId: batchForm.getFieldValue("brandId"), productId, skuId: batchForm.getFieldValue("previewSkuId") });
    ...
    const organizationPanel = (
        <div className="space-y-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                ...
                ].map((item) => { const Icon = item.icon; return <Card key={item.label}><Statistic title={<span className="inline-flex items-center gap-2"><Icon className="size-4" />{item.label}</span>} value={item.value} /></Card>; })}
            </div>
            <Card title={<span className="inline-flex items-center gap-2"><WalletCards className="size-4" />企业算力</span>} extra={canManage ? <Button size="small" icon={<Settings2 className="size-4" />} onClick={() => { creditSettingsForm.setFieldsValue({ mode: creditSummary?.mode || "personal", monthlyBudget: creditSummary?.monthlyBudget || 0, alertThreshold: creditSummary?.alertThreshold || 80 }); setCreditSettingsOpen(true); }}>额度设置</Button> : null}>
                <div className="grid gap-5 md:grid-cols-3">
                    <div><div className="text-xs text-muted-foreground">当前扣费模式</div><div className="mt-2"><Tag color={creditSummary?.mode === "shared" ? "blue" : "default"}>{creditSummary?.mode === "shared" ? "企业共享额度" : "成员个人额度"}</Tag></div></div>
                    <div><div className="text-xs text-muted-foreground">企业共享余额</div><div className="mt-1 text-2xl font-semibold tabular-nums">{(creditSummary?.balance || 0).toLocaleString()}<span className="ml-1 text-xs font-normal text-muted-foreground">点</span></div></div>
                    <div><div className="flex items-center justify-between gap-3 text-xs text-muted-foreground"><span>本月已用</span><span className="tabular-nums">{(creditSummary?.monthlyUsed || 0).toLocaleString()} / {creditSummary?.monthlyBudget ? creditSummary.monthlyBudget.toLocaleString() : "不限"}</span></div><Progress className="mt-2" percent={budgetPercent} showInfo={false} status={creditSummary?.warning ? "exception" : "normal"} /></div>
                </div>
                {creditSummary?.warning ? <Alert className="mt-4" type="warning" showIcon message={`本月算力已达到预算预警线（${creditSummary.alertThreshold}%）`} /> : null}
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4"><span className="text-sm text-muted-foreground">我的个人余额：<span className="font-medium tabular-nums text-foreground">{(creditSummary?.personalBalance || 0).toLocaleString()} 点</span></span><Button disabled={!creditSummary?.personalBalance} onClick={() => { creditTransferForm.resetFields(); setCreditTransferOpen(true); }}>从个人余额转入</Button></div>
            </Card>
            <div className="grid gap-5 xl:grid-cols-[1fr_1.6fr]">
                <Card title="企业信息" extra={canManage ? <Button size="small" icon={<Settings2 className="size-4" />} onClick={() => { organizationForm.setFieldValue("name", workspace?.organization.name); setOrganizationMode("edit"); }}>设置</Button> : null}>
    ...
    const brandPanel = (
        <div>
            ...
            <div className="mt-5 flex justify-end"><Pagination current={brandListQuery.page} pageSize={brandListQuery.pageSize} total={brandsQuery.data?.total || 0} showSizeChanger onChange={(page, pageSize) => setBrandListQuery((value) => ({ ...value, page, pageSize }))} /></div>
        </div>
    );

    const productPanel = (
        <div>
            <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-semibold">商品 / SKU 中心</h2><p className="mt-1 text-sm text-muted-foreground">以 SPU 组织卖点，以 SKU 维护规格与参考图。</p></div><div className="flex gap-2"><Input.Search allowClear placeholder="搜索名称、SPU 或类目" className="w-64" onSearch={(keyword) => setProductListQuery((value) => ({ ...value, page: 1, keyword }))} />{canWrite ? <Button type="primary" icon={<Plus className="size-4" />} onClick={() => openProduct({})}>新增商品</Button> : null}</div></div>
            <Card><Table<Product> rowKey="id" loading={productsQuery.isFetching} dataSource={products} pagination={{ current: productListQuery.page, pageSize: productListQuery.pageSize, total: productsQuery.data?.total || 0, showSizeChanger: true, onChange: (page, pageSize) => setProductListQuery((value) => ({ ...value, page, pageSize })) }} columns={[
                { title: "SPU", dataIndex: "code", width: 150, render: (value) => <span className="font-mono text-xs">{value}</span> },
                { title: "商品", render: (_, item) => <div><div className="font-medium">{item.name}</div><div className="mt-1 text-xs text-muted-foreground">{item.category || "未分类"}</div></div> },
    ...
    const batchPanel = (
        <div>
            <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-2xl font-semibold">批量生产任务</h2><p className="mt-1 text-sm text-muted-foreground">按商品与 SKU 展开服务端持久化任务项，支持审核、渠道交付和进度追踪。</p></div><div className="flex gap-2"><Input.Search allowClear placeholder="搜索任务" className="w-56" onSearch={(keyword) => setJobListQuery((value) => ({ ...value, page: 1, keyword }))} />{canWrite ? <Button type="primary" icon={<Plus className="size-4" />} onClick={() => { batchRequestId.current = crypto.randomUUID(); batchForm.resetFields(); batchForm.setFieldsValue({ deliverySpecId: "original" }); setBatchOpen(true); }}>创建任务</Button> : null}</div></div>
    ...
    const auditPanel = (
        <Card title="企业审计日志" extra={<Input.Search allowClear placeholder="搜索动作或资源" className="w-56" onSearch={(keyword) => setAuditListQuery((value) => ({ ...value, page: 1, keyword }))} />}><Table rowKey="id" loading={auditQuery.isFetching} dataSource={auditQuery.data?.items || []} pagination={{ current: auditListQuery.page, pageSize: auditListQuery.pageSize, total: auditQuery.data?.total || 0, showSizeChanger: true, onChange: (page, pageSize) => setAuditListQuery((value) => ({ ...value, page, pageSize })) }} columns={[
            { title: "时间", dataIndex: "createdAt", width: 190 }, { title: "动作", dataIndex: "action", width: 180 }, { title: "资源", render: (_, item) => `${item.resourceType} / ${item.resourceId}` }, { title: "操作者", dataIndex: "userId", width: 230 },
        ]} /></Card>
    );
    ...
    return (
        <main className="h-full overflow-y-auto bg-background">
            <div className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
                <header className="hero-atmosphere mb-9 flex min-h-64 flex-wrap items-end justify-between gap-6 rounded-[30px] bg-[#f5f5f7] p-7 sm:p-10 dark:rounded-none dark:border-b dark:border-border dark:bg-transparent dark:p-0 dark:py-12 dark:sm:py-14"><div><div className="mb-3 inline-flex items-center gap-2 text-sm font-medium text-primary"><Building2 className="size-4" /> 企业电商工作区</div><h1 className="text-5xl font-semibold tracking-[-.045em] sm:text-6xl">{workspace?.organization.name || "企业中心"}</h1><p className="mt-4 max-w-2xl text-base leading-7 text-muted-foreground">品牌资产、商品主数据与视觉生产任务，在一个工作区持续协作。</p></div><Button icon={<RefreshCw className="size-4" />} onClick={() => void invalidate()}>刷新数据</Button></header>
				<Tabs size="large" activeKey={activeTab} onChange={setActiveTab} items={[
                    ...
                    { key: "products", label: <span className="inline-flex items-center gap-2"><Boxes className="size-4" />商品 / SKU</span>, children: productPanel },
                    { key: "templates", label: <span className="inline-flex items-center gap-2"><FileStack className="size-4" />生产模板</span>, children: templatePanel },
                    { key: "batch", label: <span className="inline-flex items-center gap-2"><ClipboardList className="size-4" />批量生产</span>, children: batchPanel },
                    ...(canManage ? [{ key: "audit", label: <span className="inline-flex items-center gap-2"><History className="size-4" />审计日志</span>, children: auditPanel }] : []),
                ]} />
            </div>
            ...
            <Modal title="企业额度设置" open={creditSettingsOpen} confirmLoading={working} onCancel={() => setCreditSettingsOpen(false)} onOk={async () => { const value = await creditSettingsForm.validateFields(); const ok = await run(async () => { await updateOrganizationCreditSettings({ ...value, version: workspace?.organization.version || 0 }); await refreshUser(); }, "企业额度设置已更新"); if (ok) setCreditSettingsOpen(false); }}>
                <Form form={creditSettingsForm} layout="vertical" requiredMark={false}>
                    ...
                    <Form.Item name="monthlyBudget" label="月度预算" extra="设为 0 表示不限制。" rules={[{ required: true }]}><InputNumber className="w-full" min={0} max={1000000000} precision={0} addonAfter="点" /></Form.Item>
                    <Form.Item name="alertThreshold" label="预算预警线" rules={[{ required: true }]}><InputNumber className="w-full" min={1} max={100} precision={0} addonAfter="%" /></Form.Item>
                </Form>
            </Modal>
            <Modal title="转入企业共享额度" open={creditTransferOpen} confirmLoading={working} onCancel={() => setCreditTransferOpen(false)} onOk={async () => { const value = await creditTransferForm.validateFields(); const ok = await run(async () => { await transferOrganizationCredits(value.amount); await refreshUser(); }, "额度已转入企业共享池"); if (ok) setCreditTransferOpen(false); }}>
                <Form form={creditTransferForm} layout="vertical" requiredMark={false}><Form.Item name="amount" label="转入额度" extra={`当前个人余额 ${(creditSummary?.personalBalance || 0).toLocaleString()} 点`} rules={[{ required: true }]}><InputNumber className="w-full" min={1} max={Math.max(1, creditSummary?.personalBalance || 0)} precision={0} addonAfter="点" /></Form.Item></Form>
            </Modal>
            ...
            <Modal title="创建批量生产任务" open={batchOpen} width={720} confirmLoading={working} onCancel={() => { setBatchOpen(false); setPromptPreview(""); }} onOk={async () => { const value = await batchForm.validateFields(); const { previewSkuId: _previewSkuId, ...input } = value; if (await run(() => createBatchProductionJob({ ...input, presetVersion: input.presetVersion || 1, requestId: batchRequestId.current }), "批量任务已进入队列")) setBatchOpen(false); }}>
                <Form form={batchForm} layout="vertical">
                    <Form.Item name="name" label="任务名称" rules={[{ required: true }]}><Input placeholder="例如：秋季上新主图" /></Form.Item>
                    <Form.Item name="presetId" label="生产模板" rules={[{ required: true }]}><Select loading={templatesQuery.isFetching} options={productionTemplateOptions.map((item) => ({ value: item.id, label: `${item.title} · ${item.description}` }))} onChange={(id) => batchForm.setFieldValue("presetVersion", productionTemplateOptions.find((item) => item.id === id)?.version || 1)} /></Form.Item>
                    <Form.Item name="presetVersion" hidden><Input /></Form.Item>
                    <Form.Item name="deliverySpecId" label="渠道交付规格" rules={[{ required: true }]} extra="规格会随任务固化，并统一应用于结果像素、文件格式和压缩包命名。"><Select loading={deliverySpecsQuery.isFetching} options={(deliverySpecsQuery.data || []).map((item) => ({ value: item.id, label: item.id === "original" ? "通用 · 保留原始结果" : `${item.platform} · ${item.name} · ${item.width}×${item.height} · ${item.format.toUpperCase()}` }))} /></Form.Item>
                    <Form.Item name="brandId" label="品牌"><Select allowClear showSearch filterOption={false} onSearch={setBrandOptionKeyword} loading={brandOptionsQuery.isFetching} options={brandOptions.map((item) => ({ value: item.id, label: item.name }))} /></Form.Item>
                    <Form.Item name="productIds" label="商品" rules={[{ required: true }]}><Select mode="multiple" showSearch filterOption={false} onSearch={setProductOptionKeyword} loading={productOptionsQuery.isFetching} options={(productOptionsQuery.data?.items || []).map((item) => ({ value: item.id, label: `${item.name} · ${item.code}` }))} onChange={() => batchForm.setFieldValue("previewSkuId", undefined)} /></Form.Item>
                    {batchProductIDs[0] ? <Form.Item name="previewSkuId" label="提示词预览 SKU" extra="可选；未选择时预览商品级提示词。批量执行仍会为每个 SKU 自动注入各自属性。"><Select allowClear loading={previewSKUsQuery.isFetching} options={(previewSKUsQuery.data?.items || []).map((item) => ({ value: item.id, label: `${item.name} · ${item.code}` }))} /></Form.Item> : null}
			...
			<Drawer title={`${selectedBatch?.name || "任务"} · 结果审核`} width={1180} open={Boolean(selectedBatch)} onClose={() => { setSelectedBatch(null); setReviewDraft(null); setComparisonItems([]); setComparisonOpen(false); }}>
                ...
                <Table<BatchProductionItem> rowKey="id" loading={batchItemsQuery.isFetching} dataSource={batchItemsQuery.data?.items || []} rowSelection={{ hideSelectAll: true, selectedRowKeys: comparisonItems.map((item) => item.id), getCheckboxProps: (item) => ({ disabled: item.status !== "completed" || !item.resultStorageKey || (comparisonItems.length >= 4 && !comparisonItems.some((selected) => selected.id === item.id)) }), onSelect: (item, selected) => setComparisonItems((current) => selected ? [...current, item] : current.filter((currentItem) => currentItem.id !== item.id)) }} scroll={{ x: 1120 }} pagination={{ current: itemListQuery.page, pageSize: itemListQuery.pageSize, total: batchItemsQuery.data?.total || 0, showSizeChanger: true, onChange: (page, pageSize) => { setComparisonItems([]); setItemListQuery((value) => ({ ...value, page, pageSize })); } }} columns={[
                    ...
                    { title: "生产", width: 110, render: (_, item) => <div className="space-y-1"><Tag color={statusColors[item.status]}>{statusLabels[item.status]}</Tag><div className="text-xs text-muted-foreground">第 {item.runNumber} 轮 · {item.attempts} 次尝试</div></div> },
