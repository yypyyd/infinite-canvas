...
type dataConsistencyObject struct {
	Driver, Key, Hash, MimeType string
	Size                        int64
}
...
func InspectDataConsistency(ctx context.Context) (model.DataConsistencyReport, error) {
	...
	objects, storageStatus := listDataConsistencyObjects(ctx, snapshot)
	return inspectDataConsistencySnapshot(snapshot, objects, storageStatus), nil
}

func listDataConsistencyObjects(ctx context.Context, snapshot repository.DataConsistencySnapshot) ([]dataConsistencyObject, string) {
	setting, err := currentUserStorageSetting()
	if err != nil {
		return nil, "unconfigured"
	}
	drivers := map[string]bool{setting.Driver: true}
	for _, item := range snapshot.Files {
		drivers[effectiveStorageDriver(item.StorageDriver)] = true
	}
	...
	if drivers["qiniu"] {
		if setting.QiniuAccessKey == "" || setting.QiniuSecretKey == "" || setting.QiniuBucket == "" {
			return nil, "unconfigured"
		}
		items, listErr := listQiniuDataConsistencyObjects(ctx, setting)
		if listErr != nil {
			return nil, "error"
		}
	...
	if drivers["local"] {
		items, listErr := listLocalDataConsistencyObjects(ctx, setting)
		if listErr != nil {
			return nil, "error"
		}
		objects = append(objects, items...)
...
func listQiniuDataConsistencyObjects(ctx context.Context, setting model.StorageSetting) ([]dataConsistencyObject, error) {
	manager, marker := qiniuBucketManager(setting), ""
	objects := []dataConsistencyObject{}
	for {
		if err := ctx.Err(); err != nil {
			return nil, err
		}
		items, _, nextMarker, hasNext, err := manager.ListFiles(setting.QiniuBucket, "organizations/", "", marker, 1000)
		if err != nil {
			return nil, err
		}
		for _, item := range items {
			objects = append(objects, dataConsistencyObject{Driver: "qiniu", Key: item.Key, Hash: item.Hash, MimeType: item.MimeType, Size: item.Fsize})
		}
		if !hasNext {
			return objects, nil
		}
		if nextMarker == "" || nextMarker == marker {
			return nil, errors.New("qiniu object listing marker did not advance")
		}
		marker = nextMarker
	}
}

func listLocalDataConsistencyObjects(ctx context.Context, setting model.StorageSetting) ([]dataConsistencyObject, error) {
	...
	err = filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
		...
		if !info.Mode().IsRegular() || strings.HasPrefix(info.Name(), ".upload-") {
			return nil
		}
		...
		objects = append(objects, dataConsistencyObject{Driver: "local", Key: filepath.ToSlash(relative), Hash: hex.EncodeToString(hash.Sum(nil)), MimeType: mimeTypeByObjectPath(relative), Size: info.Size()})
		return nil
	})
	return objects, err
}
...
func inspectDataConsistencySnapshot(snapshot repository.DataConsistencySnapshot, objects []dataConsistencyObject, storageStatus string) model.DataConsistencyReport {
	...
	objectKey := func(driver, key string) string { return effectiveStorageDriver(driver) + "\x00" + key }
	...
	for _, file := range snapshot.Files {
		...
		filesByObject[objectKey(file.StorageDriver, file.ObjectKey)] = file
		if file.Size <= 0 {
	...
	for _, item := range snapshot.Reservations {
		trackedObjects[objectKey(item.StorageDriver, item.ObjectKey)] = true
	...
	if storageStatus == "error" {
		add(model.DataConsistencyIssue{Category: "object_storage", Code: "object_scan_failed", Severity: "error", ResourceType: "object_storage", ResourceID: "current", Message: "存储对象列举失败，本次未完成对象一致性检查", Target: "storage"})
	}
	if storageStatus == "ok" {
		seen := map[string]bool{}
		for _, object := range objects {
			identity := objectKey(object.Driver, object.Key)
			seen[identity] = true
			if file, ok := filesByObject[identity]; ok {
				if file.Size != object.Size || (file.Hash != "" && file.Hash != object.Hash) || assetTypeFromMime(file.MimeType) != assetTypeFromMime(object.MimeType) {
					add(model.DataConsistencyIssue{Category: "object_storage", Code: "object_metadata_mismatch", Severity: "error", OrganizationID: file.OrganizationID, ResourceType: "user_file", ResourceID: file.ID, Message: "存储对象与数据库记录的大小、哈希或媒体类型不一致", Target: file.ID})
				}
				continue
			}
			if trackedObjects[identity] {
				continue
			}
			...
			message := "企业目录中存在未被数据库登记的存储对象"
			if !organizations[organizationID] {
				message = "存储对象所属企业不存在"
			}
			target := object.Driver + ":" + object.Key
			add(model.DataConsistencyIssue{Category: "object_storage", Code: "orphan_object", Severity: "warning", OrganizationID: organizationID, ResourceType: object.Driver + "_object", ResourceID: dataConsistencyObjectID(target), Message: message, Target: target})
		}
		for identity, file := range filesByObject {
			if !seen[identity] {
				add(model.DataConsistencyIssue{Category: "object_storage", Code: "missing_object", Severity: "error", OrganizationID: file.OrganizationID, ResourceType: "user_file", ResourceID: file.ID, Message: "数据库媒体文件对应的存储对象不存在", Target: file.ID})
			}
		}
	}
