...
func generatedImageBytesWithAuthorization(ctx context.Context, item map[string]any, authorization string) ([]byte, string, error) {
	if encoded, _ := item["b64_json"].(string); strings.TrimSpace(encoded) != "" {
		...
		if comma := strings.Index(encoded, ","); comma >= 0 && strings.Contains(strings.ToLower(encoded[:comma]), "base64") {
			if prefix := strings.TrimPrefix(encoded[:comma], "data:"); strings.HasPrefix(prefix, "image/") { mimeType = strings.TrimSuffix(prefix, ";base64") }
			encoded = encoded[comma+1:]
		}
		data, err := base64.StdEncoding.DecodeString(encoded)
		if err != nil || len(data) == 0 { return nil, "", errors.New("generated image base64 is invalid") }
...
func archiveCompletedVideo(ctx context.Context, user model.AuthUser, task model.GenerationTask, request *http.Request, body []byte) ([]byte, model.UserFile, error) {
	...
	if err != nil { return body, model.UserFile{}, err }
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 { return body, model.UserFile{}, errors.New("generated video content is unavailable") }
	...
	if !strings.HasPrefix(mimeType, "video/") { return body, model.UserFile{}, errors.New("generated video metadata is invalid") }
...
func archiveVideoContentResponse(ctx context.Context, user model.AuthUser, task model.GenerationTask, response *http.Response, body []byte) (model.UserFile, error) {
	mimeType := strings.TrimSpace(strings.Split(response.Header.Get("Content-Type"), ";")[0])
	if mimeType == "" || mimeType == "application/octet-stream" { mimeType = http.DetectContentType(body) }
...
func setArchivedGenerationHeaders(w http.ResponseWriter, file model.UserFile) {
	...
	if file.Size > 0 { w.Header().Set("X-Storage-Bytes", fmt.Sprint(file.Size)) }
	if file.MimeType != "" { w.Header().Set("X-Storage-Mime-Type", file.MimeType) }
}

func attachVideoStorage(body []byte, storageKey, mimeType string, size int64) []byte {
	var payload map[string]any
	if json.Unmarshal(body, &payload) != nil { return body }
	target := payload
	if data, ok := payload["data"].(map[string]any); ok { target = data }
	target["status"] = "completed"
	target["storage_key"] = storageKey
	if mimeType != "" { target["mime_type"] = mimeType }
	if size > 0 { target["bytes"] = size }
...
func proxyArchivedGenerationContent(w http.ResponseWriter, r *http.Request, user model.AuthUser, storageKey string) bool {
	...
	if byteRange := r.Header.Get("Range"); byteRange != "" { request.Header.Set("Range", byteRange) }
	...
	for key, values := range response.Header {
		if strings.EqualFold(key, "Content-Length") { continue }
