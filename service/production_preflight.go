...
func resolveImageProduction(organizationID string, userID string, userGroup string, input model.CreateBatchProductionJobInput) (resolvedImageProduction, error) {
	...
	if len(input.ProductScopes) == 0 && len(input.ProductIDs) > 0 {
		for _, id := range input.ProductIDs {
			input.ProductScopes = append(input.ProductScopes, model.BatchProductionProductScope{ProductID: id, AllActiveSKUs: true})
		}
	}
	if len(input.TemplateSelections) == 0 && input.PresetID != "" {
		input.TemplateSelections = []model.BatchProductionTemplateSelectionInput{{TemplateID: input.PresetID, TemplateVersion: input.PresetVersion, Quantity: 1, DeliverySpecID: input.DeliverySpecID}}
	}
	if len(input.ProductScopes) == 0 || len(input.ProductScopes) > 200 {
		return resolvedImageProduction{}, safeMessageError{message: "请选择 1 到 200 个商品"}
	}
	...
	input.ProductScopes = input.ProductScopes[:0]
	input.ProductIDs = append([]string(nil), productIDs...)
	products := map[string]model.Product{}
	skusByProduct := map[string][]model.ProductSKU{}
	for _, productID := range productIDs {
		product, ok, err := repository.GetProduct(organizationID, productID)
		if err != nil {
			return resolvedImageProduction{}, err
		}
		if !ok {
			return resolvedImageProduction{}, safeMessageError{message: "任务包含不属于当前企业的商品"}
		}
		products[productID] = product
		scope := scopeByProduct[productID]
		input.ProductScopes = append(input.ProductScopes, scope)
		items, _, err := repository.ListProductSKUs(organizationID, productID, model.Query{Page: 1, PageSize: model.MaxPageSize})
		if err != nil {
			return resolvedImageProduction{}, err
		}
		...
		for _, sku := range items {
			if (scope.AllActiveSKUs && sku.Status == model.ProductStatusActive) || requested[sku.ID] {
				skusByProduct[productID] = append(skusByProduct[productID], sku)
				delete(requested, sku.ID)
			}
		}
		if len(requested) > 0 {
			return resolvedImageProduction{}, safeMessageError{message: "任务包含不属于所选商品的 SKU"}
		}
		if len(skusByProduct[productID]) == 0 {
			issues = append(issues, model.ProductionPreflightIssue{Severity: "error", Code: "SKU_REQUIRED", ProductID: productID, Field: "skuIds", Message: "商品没有符合条件的 SKU"})
		}
	}
	...
	for _, selected := range input.TemplateSelections {
		selected.TemplateID, selected.DeliverySpecID = strings.TrimSpace(selected.TemplateID), strings.TrimSpace(selected.DeliverySpecID)
		if selected.Quantity == 0 {
			selected.Quantity = 1
		}
		if selected.DeliverySpecID == "" {
			selected.DeliverySpecID = "original"
		}
		if selected.Quantity < 1 || selected.Quantity > 10 {
			return resolvedImageProduction{}, safeMessageError{message: "每个模板的结果数量必须为 1 到 10"}
		}
		prompt, version, err := resolveProductionPreset(organizationID, selected.TemplateID, selected.TemplateVersion)
		if err != nil {
			return resolvedImageProduction{}, err
		}
		...
		if definition, builtin := findBuiltinProductionTemplate(selected.TemplateID); builtin {
			...
			if !ok || template.MediaType != model.ProductionTemplateMediaTypeImage || template.Status != model.ProductionTemplateStatusActive || template.CurrentVersion <= 0 {
				return resolvedImageProduction{}, safeMessageError{message: "所选图片模板不存在、未发布或已停用"}
			}
			templateType, specJSON = template.TemplateType, templateVersion.SpecJSON
		}
		if templateType == "" {
			templateType = model.ProductionTemplateTypeCustom
		}
		_, spec, err := normalizeProductionTemplateSpec(specJSON)
		if err != nil {
			return resolvedImageProduction{}, err
		}
		delivery, err := resolveProductionDeliverySpec(selected.DeliverySpecID)
		if err != nil {
			return resolvedImageProduction{}, err
		}
		...
		if selectionKey[key] {
			return resolvedImageProduction{}, safeMessageError{message: "同一模板版本和交付规格不能重复选择"}
		}
		...
		for productID, skuItems := range skusByProduct {
			for _, sku := range skuItems {
				...
				if spec.RequireSellingPoints && len(products[productID].SellingPoints) == 0 {
					issues = append(issues, model.ProductionPreflightIssue{Severity: "error", Code: "SELLING_POINTS_REQUIRED", ProductID: productID, SKUID: sku.ID, TemplateID: selected.TemplateID, Field: "sellingPoints", Message: "商品缺少模板要求的卖点"})
				}
			}
		}
	}
	sort.Slice(resolvedSelections, func(left, right int) bool {
		...
		if a.DeliverySpecID != b.DeliverySpecID {
			return a.DeliverySpecID < b.DeliverySpecID
		}
	...
	totalSKUs := 0
	for _, items := range skusByProduct {
		totalSKUs += len(items)
	}
	totalItems := 0
	for _, selection := range selections {
		totalItems += totalSKUs * selection.Quantity
	}
	if totalItems > 5000 {
		return resolvedImageProduction{}, safeMessageError{message: "任务展开后超过 5000 个生产项，请拆分任务"}
	}
	...
	for productID, skuItems := range skusByProduct {
		for _, sku := range skuItems {
			if input.PreviewSKUID != "" && input.PreviewSKUID != sku.ID {
				continue
			}
			for _, selection := range selections {
