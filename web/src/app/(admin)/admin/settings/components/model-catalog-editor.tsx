...
export function ModelCatalogEditor({ value = [], onChange, pricingRules, onPricingRulesChange, candidateModels, channelModels }: Props) {
    ...
    const channelModelSet = useMemo(() => new Set(channelModels), [channelModels]);
    ...
    const syncChannelModels = () => {
        ...
        const additions = channelModels.filter((model) => !existing.has(model)).map((model, index) => createModel(model, inferModelModality(model), models.length + index));
        if (!additions.length) {
            message.info("渠道模型已经全部在模型中心中");
            return;
        }
        updateModels([...models, ...additions]);
        onPricingRulesChange([...pricingRules, ...additions.flatMap((model) => defaultRules(model.id, model.modality))]);
        message.success(`已同步 ${additions.length} 个模型，并生成默认计费`);
    };
    ...
    return (
        <Card
            ...
            title={
                <div>
                    ...
                    <Typography.Text type="secondary" className="ml-3 text-xs font-normal">模型信息和计费在这里统一维护，渠道只负责上游连接。</Typography.Text>
                </div>
            }
            extra={
                <Space>
                    <Button icon={<SyncOutlined />} onClick={syncChannelModels}>同步渠道模型</Button>
        ...
        >
                    ...
                    columns={[
                        {
                            ...
                            render: (_: unknown, model: AdminManagedModel) => (
                                <Flex vertical gap={2}>
                                    ...
                                    <Typography.Text type="secondary" copyable={{ text: model.id }} className="text-xs">{model.id}</Typography.Text>
                                </Flex>
                            ),
                        },
                        ...
                        },
                        {
                            ...
                            render: (_: unknown, model: AdminManagedModel) => (
                                <Typography.Text type="secondary" className="text-xs">
                                    {[capabilityLabel(model.operations), model.aspectRatios.join(" / "), model.resolutionTiers.join(" / ")].filter(Boolean).join(" · ")}
                                </Typography.Text>
                            ),
                        },
                        { title: "开放", dataIndex: "enabled", width: 76, render: (enabled: boolean, model: AdminManagedModel) => <Switch size="small" checked={enabled} onChange={(checked) => setModelEnabled(model.id, checked)} /> },
                        {
                            ...
                            render: (_: unknown, model: AdminManagedModel) => (
                                <Space size={4}>
                                    <Button type="text" size="small" icon={<EditOutlined />} onClick={() => openEdit(model)}>编辑</Button>
            ...
            <Drawer title={editingModel ? "编辑模型与计费" : "添加模型与计费"} width={720} open={drawerOpen} onClose={closeDrawer} destroyOnHidden extra={<Space><Button onClick={closeDrawer}>取消</Button><Button type="primary" onClick={() => void saveModel()}>保存模型</Button></Space>}>
                <Form form={form} layout="vertical" requiredMark={false} onValuesChange={(changed) => { if (changed.modality) changeModality(changed.modality); if (changed.resolutionTiers) changeResolutionTiers(changed.resolutionTiers); }}>
                    ...
                    <Row gutter={16}>
                        ...
                        <Col span={10}><Form.Item name="name" label="显示名称"><Input placeholder="默认与模型 ID 相同" /></Form.Item></Col>
                        <Col span={8}><Form.Item name="operations" label="模型能力" rules={[{ required: true, message: "请至少保留一项模型能力" }]} extra="系统会按模型名自动识别，仅在识别错误时修正。"><Select mode="multiple" options={operationOptions(selectedModality)} /></Form.Item></Col>
                        <Col span={3}><Form.Item name="sort" label="排序"><InputNumber precision={0} className="!w-full" /></Form.Item></Col>
                        <Col span={3}><Form.Item name="enabled" label="开放" valuePropName="checked"><Switch /></Form.Item></Col>
                        {selectedModality === "image" || selectedModality === "video" ? <Col span={12}><Form.Item name="aspectRatios" label="支持宽高比"><Select mode="tags" allowClear options={aspectRatioOptions.map((item) => ({ label: item, value: item }))} /></Form.Item></Col> : null}
                        {selectedModality === "image" || selectedModality === "video" ? <Col span={12}><Form.Item name="resolutionTiers" label="支持分辨率档" extra="每增加一个分辨率，下面会自动增加对应价格档。"><Select mode="tags" allowClear options={resolutionOptions.map((item) => ({ label: item.toUpperCase(), value: item }))} /></Form.Item></Col> : null}
                        <Col span={24}><Form.Item name="remark" label="模型备注"><Input.TextArea rows={2} /></Form.Item></Col>
                    </Row>

                    <Flex justify="space-between" align="center" gap={12} wrap style={{ margin: "8px 0 12px" }}>
                        <div>
                            <Typography.Title level={5} style={{ margin: 0 }}>计费规则</Typography.Title>
                            <Typography.Text type="secondary" className="text-xs">价格按分辨率维护；内部操作按模型能力展开，计费单位由模型类型决定。</Typography.Text>
                        </div>
                        <Tag color="blue">{capabilityLabel(selectedOperations)} · 按{unitLabel[defaultUnit(selectedModality)]}</Tag>
                    </Flex>
                    <Flex vertical gap={12}>
                        {draftRules.map((rule, index) => (
                            <Card key={rule.resolutionTier || "default"} size="small" title={rule.resolutionTier ? `${rule.resolutionTier.toUpperCase()} 价格` : "默认价格"}>
                                <Row gutter={12}>
                                    <Col span={4}><Form.Item label="分辨率"><Tag color={rule.resolutionTier ? "geekblue" : undefined}>{rule.resolutionTier ? rule.resolutionTier.toUpperCase() : "默认"}</Tag></Form.Item></Col>
                                    <Col span={6}><Form.Item label="计费方式"><Segmented block value={rule.billingMode} options={[{ label: "固定单价", value: "fixed" }, { label: "倍率", value: "ratio" }]} onChange={(value) => setRuleField(index, "billingMode", value as AdminPricingRule["billingMode"])} /></Form.Item></Col>
                                    {rule.billingMode === "fixed" ? (
                                        <Col span={6}><Form.Item label={`单价（算力点/${unitLabel[defaultUnit(selectedModality)]}）`}><InputNumber min={0} precision={0} className="!w-full" value={rule.credits} onChange={(value) => setRuleField(index, "credits", Number(value) || 0)} /></Form.Item></Col>
                                    ) : (
                                        <Col span={6}><Form.Item label="模型倍率"><InputNumber min={0.0001} step={0.1} className="!w-full" value={rule.modelRatio} onChange={(value) => setRuleField(index, "modelRatio", Number(value) || 1)} /></Form.Item></Col>
                                    )}
                                    <Col span={5}><Form.Item label="最低消费"><InputNumber min={0} precision={0} className="!w-full" value={rule.minCredits} onChange={(value) => setRuleField(index, "minCredits", Number(value) || 0)} /></Form.Item></Col>
                                    <Col span={3}><Form.Item label="启用"><Switch checked={rule.enabled} onChange={(checked) => setRuleField(index, "enabled", checked)} /></Form.Item></Col>
                                </Row>
                            </Card>
                        ))}
                    </Flex>
        ...
        <Space size={[4, 4]} wrap>
            {enabled.slice(0, 3).map((rule, index) => (
                <Tag key={`${index}-${rule.resolutionTier}`} color={rule.billingMode === "ratio" ? "blue" : "green"}>
                    {rule.resolutionTier ? rule.resolutionTier.toUpperCase() : "默认"} · {rule.billingMode === "ratio" ? `×${rule.modelRatio}` : `${rule.credits} 点/${unitLabel[rule.unit] || rule.unit}`}
                </Tag>
            ))}
...
function normalizeModels(models: AdminManagedModel[]) {
    ...
    return models
        .map((model, index) => {
            ...
            return { ...model, id: model.id.trim(), name: model.name?.trim() || model.id.trim(), modality, operations: normalizeModelOperations(model.operations, model.id, modality), enabled: model.enabled !== false, sort: Number(model.sort ?? index) || 0, aspectRatios: supportsResolution ? unique(model.aspectRatios) : [], resolutionTiers: supportsResolution ? unique(model.resolutionTiers) : [], remark: model.remark || "" };
        })
        .filter((model) => model.id && !seen.has(model.id) && seen.add(model.id))
        .sort((a, b) => (a.sort === b.sort ? a.id.localeCompare(b.id) : a.sort - b.sort));
}

function defaultRules(model: string, modality: string) {
    return expandPricingTiers([defaultRule(model, modality)], createModel(model, modality, 0));
}

function pricingTiers(rules: AdminPricingRule[], model: AdminManagedModel) {
    const byResolution = new Map(uniquePricingTiers(rules).map((rule) => [rule.resolutionTier, rule]));
    const fallback = byResolution.get("") || defaultRule(model.id, model.modality);
    return ["", ...unique(model.resolutionTiers)].map((resolutionTier) => normalizeRule({ ...(byResolution.get(resolutionTier) || fallback), model: model.id || "", modality: model.modality, operation: defaultOperation(model.modality), unit: defaultUnit(model.modality), resolutionTier, remark: "" }));
}
...
function uniquePricingTiers(rules: AdminPricingRule[]) {
    ...
    for (const rule of rules) {
        const resolution = rule.resolutionTier.trim().toLowerCase();
        const current = result.get(resolution);
        if (!current || rule.operation === "generation") result.set(resolution, normalizeRule(rule));
    }
...
function defaultRule(model: string, modality: string): AdminPricingRule {
    return { model, modality, operation: defaultOperation(modality), unit: defaultUnit(modality), resolutionTier: "", billingMode: "fixed", credits: 1, minCredits: 0, modelRatio: 1, completionRatio: 1, enabled: true, remark: "" };
}

function normalizeRule(rule: AdminPricingRule): AdminPricingRule {
    return { ...rule, model: rule.model.trim(), modality: rule.modality.trim().toLowerCase(), operation: rule.operation.trim().toLowerCase(), unit: rule.unit.trim().toLowerCase(), resolutionTier: rule.resolutionTier.trim().toLowerCase(), credits: Math.max(0, Number(rule.credits) || 0), minCredits: Math.max(0, Number(rule.minCredits) || 0), modelRatio: Math.max(0, Number(rule.modelRatio) || 1), completionRatio: Math.max(0, Number(rule.completionRatio) || 1), remark: rule.remark || "" };
}
