...
export default function CommerceLayout({ children }: { children: ReactNode }) {
    ...
    return <div className="flex min-h-[calc(100vh-64px)] bg-[var(--ant-color-bg-layout)]">
        <aside className="sticky top-16 hidden h-[calc(100vh-64px)] w-60 shrink-0 border-r border-[var(--ant-color-border-secondary)] bg-[var(--ant-color-bg-container)] p-4 xl:block">
            ...
            <nav className="space-y-1">{links.map(([href, label, Icon]) => {
                ...
                return <Link key={href} href={href} className={`flex min-h-10 items-center gap-3 rounded-lg px-3 text-sm transition ${active ? "bg-[var(--ant-color-primary-bg)] text-[var(--ant-color-primary)]" : "text-[var(--ant-color-text)] hover:bg-[var(--ant-color-fill-tertiary)]"}`}><Icon size={17}/>{label}</Link>;
            })}</nav>
