...
func TestBatchJobHTTPCreateIsIdempotentAndTenantScoped(t *testing.T) {
	...
	if err := database.Create(&resultFile).Error; err != nil { t.Fatal(err) }
	if err := database.Model(&model.BatchProductionItem{}).Where("organization_id = ? AND job_id = ?", tenant.Organization.ID, job.ID).Updates(map[string]any{"status": model.BatchProductionStatusCompleted, "result_storage_key": resultFile.StorageKey}).Error; err != nil { t.Fatal(err) }
...
func TestBatchItemHTTPReviewRetryAndPrimaryWorkflow(t *testing.T) {
	...
	if err := database.Create(&skus).Error; err != nil { t.Fatal(err) }
	...
	if err := database.Create(&model.OrganizationMember{ID: "member-router-batch-reviewer", OrganizationID: tenant.Organization.ID, UserID: reviewer.ID, Role: model.OrganizationRoleReviewer, Version: 1, CreatedAt: "3", UpdatedAt: "3"}).Error; err != nil { t.Fatal(err) }
	...
	if err := database.Create(&member).Error; err != nil { t.Fatal(err) }
	if err := database.Create(&model.OrganizationMember{ID: "member-router-batch-member", OrganizationID: tenant.Organization.ID, UserID: member.ID, Role: model.OrganizationRoleMember, Version: 1, CreatedAt: "3", UpdatedAt: "3"}).Error; err != nil { t.Fatal(err) }
	...
	var job model.BatchProductionJob
	if created.Code != 0 || json.Unmarshal(created.Data, &job) != nil { t.Fatalf("create review job response: %#v", created) }
	var items []model.BatchProductionItem
	if err := database.Where("job_id = ?", job.ID).Order("sku_id asc").Find(&items).Error; err != nil || len(items) != 2 { t.Fatalf("review items: %#v, err=%v", items, err) }
	for index := range items {
		if err := database.Model(&model.BatchProductionItem{}).Where("id = ?", items[index].ID).Updates(map[string]any{"status": model.BatchProductionStatusCompleted, "result_storage_key": "image:router-batch-review-" + items[index].ID, "review_status": model.BatchProductionReviewPending, "attempts": 1, "finished_at": "4"}).Error; err != nil { t.Fatal(err) }
	}
	if err := database.Model(&model.BatchProductionJob{}).Where("id = ?", job.ID).Updates(map[string]any{"status": model.BatchProductionStatusCompleted, "completed_items": 2}).Error; err != nil { t.Fatal(err) }

	itemURL := func(baseURL string, item model.BatchProductionItem, action string) string { return baseURL + "/api/commerce/batch-jobs/" + job.ID + "/items/" + item.ID + "/" + action }
	if response := routerTestJSON(t, memberClient, http.MethodPost, itemURL(memberURL, items[0], "review"), map[string]any{"runNumber": 1, "status": model.BatchProductionReviewApproved, "comment": "越权审核"}, headers); response.Code != 1 { t.Fatalf("member review response: %#v", response) }
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, itemURL(reviewerURL, items[1], "review"), map[string]any{"runNumber": 1, "status": model.BatchProductionReviewRejected, "comment": ""}, headers); response.Code != 1 { t.Fatalf("empty reject comment response: %#v", response) }
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, itemURL(reviewerURL, items[0], "review"), map[string]any{"runNumber": 1, "status": model.BatchProductionReviewApproved, "comment": "可交付"}, headers); response.Code != 0 { t.Fatalf("approve response: %#v", response) }
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, itemURL(reviewerURL, items[0], "primary"), map[string]int{"runNumber": 1}, headers); response.Code != 0 { t.Fatalf("primary response: %#v", response) }
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, itemURL(reviewerURL, items[1], "review"), map[string]any{"runNumber": 1, "status": model.BatchProductionReviewRejected, "comment": "商品角度不正确"}, headers); response.Code != 0 { t.Fatalf("reject response: %#v", response) }
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, itemURL(reviewerURL, items[1], "primary"), map[string]int{"runNumber": 1}, headers); response.Code != 1 { t.Fatalf("rejected primary response: %#v", response) }
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, itemURL(reviewerURL, items[1], "retry"), map[string]int{"runNumber": 1}, headers); response.Code != 0 { t.Fatalf("rejected retry response: %#v", response) }
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, itemURL(reviewerURL, items[1], "review"), map[string]any{"runNumber": 1, "status": model.BatchProductionReviewApproved, "comment": "旧页面"}, headers); response.Code != 1 { t.Fatalf("stale review response: %#v", response) }

	var retried model.BatchProductionItem
	if err := database.First(&retried, "id = ?", items[1].ID).Error; err != nil { t.Fatal(err) }
	if retried.Status != model.BatchProductionStatusQueued || retried.RunNumber != 2 || retried.ResultStorageKey != "" || retried.ReviewStatus != "" || retried.ReviewComment != "" { t.Fatalf("unexpected retried review item: %#v", retried) }
	if err := database.Model(&model.BatchProductionItem{}).Where("id = ?", retried.ID).Updates(map[string]any{"status": model.BatchProductionStatusCompleted, "result_storage_key": "image:router-batch-review-new", "review_status": model.BatchProductionReviewPending, "attempts": 1, "finished_at": "6"}).Error; err != nil { t.Fatal(err) }
	if err := database.Model(&model.BatchProductionJob{}).Where("id = ?", job.ID).Updates(map[string]any{"status": model.BatchProductionStatusCompleted, "completed_items": 2}).Error; err != nil { t.Fatal(err) }
