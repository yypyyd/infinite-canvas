...
func CreateExpandedBatchProductionJob(job model.BatchProductionJob, scopes []model.BatchProductionProductScope, selections []model.BatchProductionTemplateSelection, itemEstimatedCredits map[string]int, auditLogs ...model.OrganizationAuditLog) (model.BatchProductionJob, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		productIDs := make([]string, 0, len(scopes)); scopeByProduct := map[string]model.BatchProductionProductScope{}
		for _, scope := range scopes { productIDs = append(productIDs, scope.ProductID); scopeByProduct[scope.ProductID] = scope }; sort.Strings(productIDs)
		var products []model.Product
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("organization_id = ? AND id IN ?", job.OrganizationID, productIDs).Find(&products).Error; err != nil { return err }
		if len(products) != len(productIDs) { return gorm.ErrRecordNotFound }
		var skus []model.ProductSKU
		if err := tx.Where("organization_id = ? AND product_id IN ?", job.OrganizationID, productIDs).Order("product_id asc, id asc").Find(&skus).Error; err != nil { return err }
		selectedSKUs := []model.ProductSKU{}
		for _, sku := range skus { scope := scopeByProduct[sku.ProductID]; match := scope.AllActiveSKUs && sku.Status == model.ProductStatusActive; for _, id := range scope.SKUIDs { if id == sku.ID { match = true; break } }; if match { selectedSKUs = append(selectedSKUs, sku) } }
		items := make([]model.BatchProductionItem, 0)
		for selectionIndex := range selections { selection := &selections[selectionIndex]; selection.ID = fmt.Sprintf("%s-selection-%d", job.ID, selectionIndex+1); selection.OrganizationID, selection.JobID, selection.CreatedAt = job.OrganizationID, job.ID, job.CreatedAt }
		for _, sku := range selectedSKUs { for selectionIndex := range selections { selection := &selections[selectionIndex]; estimatedCredits, ok := itemEstimatedCredits[fmt.Sprintf("%s\x00%d", sku.ID, selection.SelectionIndex)]; if !ok { return errors.New("batch production item estimate is missing") }; for variant := 1; variant <= selection.Quantity; variant++ { items = append(items, model.BatchProductionItem{ID: fmt.Sprintf("%s-item-%d", job.ID, len(items)+1), OrganizationID: job.OrganizationID, JobID: job.ID, ProductID: sku.ProductID, SKUID: sku.ID, TemplateSelectionID: selection.ID, TemplateID: selection.TemplateID, TemplateVersion: selection.TemplateVersion, TemplateType: selection.TemplateType, VariantIndex: variant, EstimatedCredits: estimatedCredits, Status: model.BatchProductionStatusQueued, RunNumber: 1, CreatedAt: job.CreatedAt, UpdatedAt: job.UpdatedAt}) } } }
		if len(items) == 0 { return gorm.ErrRecordNotFound }
		...
		snapshots := []model.BatchProductionSnapshot{}; snapshotSize := 0
		appendSnapshot := func(kind, resourceID string, value any) (string, error) { data, err := json.Marshal(value); if err != nil { return "", err }; snapshotSize += len(data); if snapshotSize > 16<<20 { return "", ErrBatchProductionSnapshotTooLarge }; id := job.ID+":"+kind+":"+resourceID; snapshots = append(snapshots, model.BatchProductionSnapshot{ID: id, OrganizationID: job.OrganizationID, JobID: job.ID, Kind: kind, ResourceID: resourceID, Data: string(data), Size: len(data), CreatedAt: job.CreatedAt}); return id, nil }
		productSnapshots := map[string]string{}; for _, product := range products { id, err := appendSnapshot("product", product.ID, product); if err != nil { return err }; productSnapshots[product.ID] = id }
		skuSnapshots := map[string]string{}; for _, sku := range selectedSKUs { id, err := appendSnapshot("sku", sku.ID, sku); if err != nil { return err }; skuSnapshots[sku.ID] = id }
		for index := range selections { snapshotID, err := appendSnapshot("template", selections[index].ID, selections[index]); if err != nil { return err }; selections[index].TemplateSnapshotID = snapshotID }
		_, err := appendSnapshot("request", job.RequestID, map[string]any{"productScopes": scopes, "templateSelections": selections}); if err != nil { return err }
		for index := range items { items[index].ProductSnapshotID, items[index].SKUSnapshotID = productSnapshots[items[index].ProductID], skuSnapshots[items[index].SKUID] }
		inputStorageKeys := []string{}
		if job.BrandID != "" {
			var brand model.Brand
			if err := tx.Where("organization_id = ? AND id = ?", job.OrganizationID, job.BrandID).First(&brand).Error; err != nil { return err }
			brandID, err := appendSnapshot("brand", brand.ID, brand); if err != nil { return err }
			if brand.LogoStorageKey != "" { inputStorageKeys = append(inputStorageKeys, brand.LogoStorageKey) }
			for index := range items { items[index].BrandSnapshotID = brandID }
		}
		if err := tx.Create(&job).Error; err != nil { return err }
		if err := tx.CreateInBatches(&selections, 100).Error; err != nil { return err }
		if err := tx.CreateInBatches(&snapshots, 200).Error; err != nil { return err }
		if err := tx.CreateInBatches(&items, 200).Error; err != nil { return err }
		for _, sku := range selectedSKUs { inputStorageKeys = append(inputStorageKeys, sku.ImageStorageKeys...) }
...
func reserveBatchProductionCredits(tx *gorm.DB, organization *model.Organization, job *model.BatchProductionJob) error {
	...
	if organization.CreditMode == model.OrganizationCreditModeShared {
		...
		if organization.Credits-organization.ReservedCredits < job.EstimatedCredits { return ErrInsufficientOrganizationCredits }
		if organization.MonthlyCreditBudget > 0 && organization.MonthlyCreditsUsed+organization.ReservedCredits+job.EstimatedCredits > organization.MonthlyCreditBudget { return ErrOrganizationCreditBudgetExceeded }
	...
	var user model.User
	if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("id = ?", job.CreatedBy).First(&user).Error; err != nil { return err }
	if user.Credits-user.ReservedCredits < job.EstimatedCredits { return ErrInsufficientUserCredits }
...
func GetBatchProductionJob(organizationID, id string) (model.BatchProductionJob, []model.BatchProductionTemplateSelection, bool, error) {
	db, err := DB(); if err != nil { return model.BatchProductionJob{}, nil, false, err }
	var job model.BatchProductionJob
	if err := db.Where("organization_id = ? AND id = ?", organizationID, id).First(&job).Error; errors.Is(err, gorm.ErrRecordNotFound) { return job, nil, false, nil } else if err != nil { return job, nil, false, err }
...
func GetBatchProductionTemplateSelection(organizationID, jobID, id string) (model.BatchProductionTemplateSelection, bool, error) {
	db, err := DB(); if err != nil { return model.BatchProductionTemplateSelection{}, false, err }
	...
	if errors.Is(err, gorm.ErrRecordNotFound) { return selection, false, nil }
