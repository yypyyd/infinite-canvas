...
func ReserveUserFileUpload(item model.UserFileUploadReservation, quota int64, timestamp string) (model.UserFileUploadReservation, error) {
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("organization_id = ? AND storage_key = ?", item.OrganizationID, item.StorageKey).First(&previous).Error; err == nil {
			if err := queueUserObjectDeletion(tx, previous.OrganizationID, previous.UserID, previous.ObjectKey, previous.Size, previous.CleanupAfter, timestamp); err != nil {
				return err
			}
	...
	err = db.Transaction(func(tx *gorm.DB) error {
		...
		if result.Error == nil && (!reservation.ReplaceExisting || reservation.ReplaceObjectKey != existing.ObjectKey) {
			...
			if existing.ObjectKey != reservation.ObjectKey {
				if err := queueUserObjectDeletion(tx, reservation.OrganizationID, reservation.UserID, reservation.ObjectKey, reservation.Size, timestamp, timestamp); err != nil {
					return err
				}
			}
		...
		if result.Error == nil {
			...
			if existing.ObjectKey != reservation.ObjectKey {
				if err := queueUserObjectDeletion(tx, existing.OrganizationID, existing.UserID, existing.ObjectKey, existing.Size, timestamp, timestamp); err != nil {
					return err
				}
			}
			if err := tx.Model(&model.UserFile{}).Where("organization_id = ? AND id = ?", organizationID, existing.ID).Updates(map[string]any{"user_id": userID, "object_key": item.ObjectKey, "hash": hash, "mime_type": mimeType, "size": size, "unreferenced_at": unreferencedAt, "updated_at": timestamp}).Error; err != nil {
				return err
			}
...
func CollectUserFileGarbage(organizationID string, timestamp string, cutoff string, quota int64) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		for _, file := range files {
			...
			if err := queueUserObjectDeletion(tx, file.OrganizationID, file.UserID, file.ObjectKey, file.Size, timestamp, timestamp); err != nil {
				return err
			}
...
func CancelUserFileUploadReservation(organizationID string, userID string, reservationID string, timestamp string) error {
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := queueUserObjectDeletion(tx, reservation.OrganizationID, reservation.UserID, reservation.ObjectKey, reservation.Size, reservation.CleanupAfter, timestamp); err != nil {
			return err
		}
		if err := tx.Delete(&reservation).Error; err != nil {
			return err
...
func expireUserFileUploadReservations(tx *gorm.DB, organizationID string, timestamp string) error {
	...
	for _, item := range items {
		if err := queueUserObjectDeletion(tx, item.OrganizationID, item.UserID, item.ObjectKey, item.Size, item.CleanupAfter, timestamp); err != nil {
			return err
		}
	}
