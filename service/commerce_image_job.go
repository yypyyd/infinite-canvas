...
func CreateBatchProductionJob(user model.AuthUser, input model.CreateBatchProductionJobInput) (model.BatchProductionJob, error) {
	if len(input.ProductScopes) == 0 && len(input.TemplateSelections) == 0 {
		return legacyCreateBatchProductionJob(user, input)
	}
	...
	if !resolved.Preflight.CanSubmit {
		return model.BatchProductionJob{}, safeMessageError{message: "图片生产预检未通过，请修复商品、SKU 或模板问题"}
	}
	...
	hashPayload, err := json.Marshal(struct {
		...
		Scopes    []model.BatchProductionProductScope           `json:"productScopes"`
		Templates []model.BatchProductionTemplateSelectionInput `json:"templateSelections"`
	...
	if errors.Is(err, repository.ErrBatchProductionSnapshotTooLarge) {
		return result, safeMessageError{message: "批量任务输入快照总量不能超过 16MB"}
	}
	if errors.Is(err, repository.ErrBatchProductionRequestConflict) {
		return result, safeMessageError{message: "任务请求编号已用于不同的请求内容"}
	}
	if errors.Is(err, repository.ErrBatchProductionOrganizationQueueFull) {
		return result, safeMessageError{message: "企业待生产项目已达上限，请等待现有任务完成"}
	}
...
func GetBatchProductionJob(user model.AuthUser, id string) (map[string]any, error) {
	...
	job, selections, ok, err := repository.GetBatchProductionJob(organization.ID, strings.TrimSpace(id))
	if err != nil {
		return nil, err
	}
	if !ok {
		return nil, safeMessageError{message: "批量任务不存在"}
	}
