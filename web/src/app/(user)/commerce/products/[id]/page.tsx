...
export default function CommerceProductDetailPage() {
    ...
    const [draft, setDraft] = useState<Partial<ProductSKU> | null>(null);
    ...
    async function save() {
        ...
        try {
            attributes = JSON.parse(value.attributesText || "{}") as Record<string, string>;
        } catch {
        ...
        setWorking(true);
        try {
            const imageStorageKeys = String(value.imageStorageKeysText || "")
                .split("\n")
                .map((item) => item.trim())
                .filter(Boolean);
            await saveCommerceProductSKU({ ...draft, productId: id, code: value.code, name: value.name, status: value.status, attributes, imageStorageKeys });
            message.success("SKU 已保存");
            setDraft(null);
            await refresh();
        } catch (error) {
            message.error(error instanceof Error ? error.message : "保存 SKU 失败");
        } finally {
            setWorking(false);
        }
    }
    async function remove(item: ProductSKU) {
        setWorking(true);
        try {
            await deleteCommerceProductSKU(item.id, item.version);
            message.success("SKU 已删除");
            await refresh();
        } catch (error) {
            message.error(error instanceof Error ? error.message : "删除 SKU 失败");
        } finally {
            setWorking(false);
        }
    }
    return (
        <div className="space-y-4">
            <Card
                ...
                title={product.data?.name || "商品详情"}
                extra={
                    <Link href={`/commerce/production/images?productIds=${id}`}>
            ...
            >
                <Descriptions
                    column={3}
                    items={[
                        { key: "code", label: "SPU", children: product.data?.code },
                        { key: "brand", label: "品牌", children: product.data?.brandName || "-" },
                        { key: "category", label: "类目", children: product.data?.category || "-" },
                        { key: "status", label: "状态", children: <Tag>{product.data?.status}</Tag> },
                        { key: "description", label: "描述", span: 2, children: product.data?.description || "-" },
                        { key: "sellingPoints", label: "卖点", span: 3, children: product.data?.sellingPoints?.join("；") || "-" },
                    ]}
                />
            </Card>
            <Card
                ...
                extra={
                    canWrite ? (
                        <Button type="primary" onClick={() => openSKU()}>
                            新增 SKU
                        </Button>
                    ) : null
                }
            >
                <Table
                    rowKey="id"
                    dataSource={skus.data?.items || []}
                    loading={skus.isLoading}
                    scroll={{ x: 900 }}
                    pagination={false}
                    columns={[
                        { title: "SKU", dataIndex: "name" },
                        ...
                        {
                            title: "参考图",
                            dataIndex: "imageStorageKeys",
                            render: (items: string[]) =>
                                items?.length ? (
                                    <Image.PreviewGroup>
                                        <Space>
                                            {items.slice(0, 4).map((key) => (
                                                <Image key={key} src={workspaceFileUrl(key)} width={44} height={44} className="object-cover" />
                                            ))}
                                        </Space>
                                    </Image.PreviewGroup>
                                ) : (
                        ...
                        },
                        {
                            ...
                            render: (_, record) =>
                                canWrite ? (
                                    <Space>
                                        <Button size="small" onClick={() => openSKU(record)}>
                                            编辑
                                        </Button>
                                        <Popconfirm title="确认删除 SKU？" onConfirm={() => void remove(record)}>
            ...
            <Modal title={draft?.id ? "编辑 SKU" : "新增 SKU"} open={Boolean(draft)} confirmLoading={working} onCancel={() => setDraft(null)} onOk={() => void save()} width={680}>
                <Form form={form} layout="vertical">
                    ...
                    <Form.Item name="imageStorageKeysText" label="参考图 StorageKey" extra="每行一个已上传到当前企业的文件 StorageKey，服务端会验证归属与上限。">
