...
func CreateBatchProductionArchive(ctx context.Context, user model.AuthUser, jobID string) (BatchProductionArchive, error) {
	...
	if err != nil {
		return BatchProductionArchive{}, err
	}
	jobID = strings.TrimSpace(jobID)
	job, items, err := repository.ListBatchProductionArchiveItems(organization.ID, jobID)
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return BatchProductionArchive{}, safeMessageError{message: "批量任务不存在"}
	}
	if err != nil {
		return BatchProductionArchive{}, err
	}
	if len(items) == 0 {
		return BatchProductionArchive{}, safeMessageError{message: "当前任务没有审核通过的结果"}
	}
	var totalSize int64
	for _, item := range items {
		if item.Size <= 0 || item.Size > maxUserFileSize || totalSize > maxBatchProductionArchiveBytes-item.Size {
			return BatchProductionArchive{}, safeMessageError{message: "审核通过的结果总量过大，请分批下载"}
		}
		totalSize += item.Size
	}
	...
	if err := writeBatchProductionArchive(ctx, organization.ID, file, job.DeliverySpec, items); err != nil {
		archive.Cleanup()
		return BatchProductionArchive{}, err
	}
	info, err := file.Stat()
...
func writeBatchProductionArchive(ctx context.Context, organizationID string, target io.Writer, spec model.ProductionDeliverySpec, items []model.BatchProductionArchiveItem) error {
	...
	client := &http.Client{
		...
		CheckRedirect: func(request *http.Request, via []*http.Request) error {
			if len(via) >= 5 || !validHTTPSURL(request.URL.String()) {
				return errors.New("batch archive redirect is not allowed")
			}
			return nil
		},
	}
	...
	for _, item := range items {
		...
		if item.TemplateSelectionID != "" {
			if item.ResolvedTemplateSelectionID != item.TemplateSelectionID {
				_ = writer.Close()
				return errors.New("batch archive template selection is unavailable")
			}
			resolvedSpec, err := resolveProductionDeliverySpec(item.DeliverySpec.ID)
			if err != nil || resolvedSpec != item.DeliverySpec {
				_ = writer.Close()
				return errors.New("batch archive delivery specification is invalid")
			}
		...
		stored, exists, lookupErr := repository.GetUserFile(organizationID, item.ResultStorageKey)
		if lookupErr != nil || !exists {
			_ = writer.Close()
			return errors.New("batch archive source is unavailable")
		}
		if effectiveStorageDriver(stored.StorageDriver) == "local" {
			setting, settingErr := currentUserStorageSetting()
			if settingErr != nil {
				_ = writer.Close()
				return settingErr
			}
			path, pathErr := localStorageObjectPath(setting, stored.ObjectKey)
			if pathErr != nil {
				_ = writer.Close()
				return pathErr
			}
			...
			written, copyErr := io.CopyN(entry, source, item.Size+1)
			_ = source.Close()
			if written != item.Size || (copyErr != nil && !errors.Is(copyErr, io.EOF)) {
				_ = writer.Close()
				return errors.New("batch archive source size changed")
			}
			continue
		}
...
func batchProductionArchiveName(job model.BatchProductionJob) string {
	...
	if name == "" {
		name = safeBatchArchiveSegment(job.ID)
	}
	if name == "" {
		name = "batch-results"
	}
...
func batchProductionArchiveEntryName(item model.BatchProductionArchiveItem, spec model.ProductionDeliverySpec) string {
	...
	if product == "" {
		product = safeBatchArchiveSegment(item.ProductID)
	}
	if product == "" {
		product = "product"
	}
	...
	if sku == "" && item.SKUID != "" {
		sku = safeBatchArchiveSegment(item.SKUID)
	}
	if sku == "" {
		sku = "SPU"
	}
	itemSegment := safeBatchArchiveSegment(item.ID)
	if runes := []rune(itemSegment); len(runes) > 10 {
		itemSegment = string(runes[len(runes)-10:])
	}
	if itemSegment == "" {
		itemSegment = stableBatchArchiveSuffix(item.ID)
	}
	role := "结果"
	if item.IsPrimary {
		role = "主图"
	...
	if strings.TrimSpace(pattern) == "" {
		pattern = "{spu}_{sku}_{role}_{item}"
	}
	templateType := safeBatchArchiveSegment(string(item.TemplateType))
	if templateType == "" {
		templateType = "legacy"
	}
	variant := fmt.Sprintf("%d", item.VariantIndex)
	if item.VariantIndex < 1 {
		variant = "1"
	}
	for key, value := range map[string]string{"{spu}": product, "{sku}": sku, "{role}": role, "{template}": templateType, "{variant}": variant, "{item}": itemSegment} {
		pattern = strings.ReplaceAll(pattern, key, value)
	}
	name := safeBatchArchiveSegment(pattern)
	if name == "" {
		name = sku + "_" + templateType + "_" + variant
	}
	uniqueSuffix := stableBatchArchiveSuffix(item.ID)
	maxBaseRunes := 80 - 1 - len([]rune(uniqueSuffix))
	nameRunes := []rune(name)
	if len(nameRunes) > maxBaseRunes {
		name = string(nameRunes[:maxBaseRunes])
	}
...
func uniqueBatchArchiveEntryName(candidate string, used map[string]int) string {
	...
	if index := strings.LastIndex(candidate, "/"); index >= 0 {
		directory, filename = candidate[:index+1], candidate[index+1:]
	}
	extension := ""
	if index := strings.LastIndex(filename, "."); index >= 0 {
		filename, extension = filename[:index], filename[index:]
	}
	protectedSuffix := ""
	if index := strings.LastIndex(filename, "_"); index >= 0 && len(filename[index+1:]) == 12 {
		if _, err := hex.DecodeString(filename[index+1:]); err == nil {
			protectedSuffix, filename = filename[index:], filename[:index]
		}
	}
	for sequence := used[candidate] + 1; ; sequence++ {
		...
		if len(prefixRunes) > maxPrefixRunes {
			prefixRunes = prefixRunes[:maxPrefixRunes]
		}
