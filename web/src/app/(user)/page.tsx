    ...
    return (
        <main className="h-full overflow-y-auto bg-background text-foreground">
            <section className="px-4 pb-4 pt-3 sm:px-6 lg:px-8">
                <div className="relative mx-auto min-h-[680px] max-w-[1440px] overflow-hidden rounded-[32px] bg-[#f5f5f7] px-6 py-16 text-center dark:bg-[#1d1d1f] sm:px-10 lg:py-20">
                    <div className="relative z-10 mx-auto max-w-4xl">
                        ...
                        <div className="mt-8 flex flex-wrap justify-center gap-3">
                            <Link href="/image?preset=product-main" className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#0071e3] px-6 text-sm font-medium text-white transition hover:bg-[#0077ed] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0071e3] focus-visible:ring-offset-2">
                                制作商品主图 <ArrowRight className="size-4" />
                            </Link>
                            <Link href="/canvas" className="inline-flex min-h-11 items-center gap-2 rounded-full border border-[#0071e3] px-6 text-sm font-medium text-[#0071e3] transition hover:bg-[#0071e3] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0071e3] focus-visible:ring-offset-2 dark:border-[#2997ff] dark:text-[#2997ff] dark:hover:bg-[#2997ff] dark:hover:text-[#101012]">
                    ...
                    <div className="relative mx-auto mt-12 aspect-[16/7] w-full max-w-5xl overflow-hidden rounded-[28px] bg-[linear-gradient(145deg,#e9e4dc,#fbfbfd_54%,#dbe8f5)] shadow-[0_34px_90px_rgba(29,29,31,.13)] dark:bg-[linear-gradient(145deg,#303033,#1d1d1f_54%,#17324b)]">
            ...
            <section className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 lg:px-8">
                <div className="mb-10 max-w-3xl">
                    <p className="text-sm font-medium text-[#0071e3] dark:text-[#2997ff]">从任务开始</p>
                    ...
                    {commercePresets.map((preset, index) => {
                        ...
                        return (
                            <Link key={preset.id} href={`/image?preset=${preset.id}`} className="group flex min-h-64 flex-col rounded-[28px] bg-card p-7 shadow-[0_1px_0_rgba(0,0,0,.04),0_16px_50px_rgba(29,29,31,.06)] ring-1 ring-black/[.04] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_65px_rgba(29,29,31,.11)] dark:ring-white/10">
                                ...
                                <div className="mt-auto pt-10"><h3 className="text-xl font-semibold tracking-tight">{preset.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{preset.description}</p><span className="mt-5 inline-flex items-center gap-1 text-sm font-medium text-[#0071e3] dark:text-[#2997ff]">立即制作 <ArrowRight className="size-4 transition group-hover:translate-x-1" /></span></div>
                            </Link>
                        );
                    })}
                </div>
            ...
            <section className="px-4 pb-4 sm:px-6 lg:px-8">
                <div className="mx-auto max-w-[1440px] overflow-hidden rounded-[32px] bg-[#1d1d1f] px-6 py-16 text-white sm:px-10 lg:px-14">
                    <div className="grid gap-12 lg:grid-cols-[.8fr_2fr]">
                        <div><Play className="size-6 text-[#2997ff]" /><h2 className="mt-6 text-4xl font-semibold tracking-[-.04em]">更短的上新流程。</h2><p className="mt-4 text-base leading-7 text-white/55">减少工具切换，把时间留给选片和创意判断。</p></div>
                        <div className="grid gap-8 md:grid-cols-3">{workflow.map((item) => <div key={item.step} className="border-t border-white/20 pt-5"><span className="text-sm text-[#2997ff]">{item.step}</span><h3 className="mt-8 text-xl font-medium">{item.title}</h3><p className="mt-3 text-sm leading-6 text-white/55">{item.detail}</p></div>)}</div>
                    </div>
                </div>
            </section>
            ...
            {promptShowcase.length ? (
                <section className="mx-auto max-w-[1440px] px-4 py-20 sm:px-6 lg:px-8">
                    <div className="mb-9 flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-medium text-[#0071e3] dark:text-[#2997ff]">视觉灵感</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.04em]">看看还能怎么呈现商品。</h2></div><Link href="/prompts" className="inline-flex items-center gap-1 text-sm text-[#0071e3] dark:text-[#2997ff]">查看全部 <ArrowRight className="size-4" /></Link></div>
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{promptShowcase.map((item, index) => <button key={item.id} type="button" onClick={() => { setPreviewIndex(index); setPreviewOpen(true); }} className="group relative aspect-[4/3] overflow-hidden rounded-[24px] bg-muted text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0071e3] focus-visible:ring-offset-2"><img src={item.coverUrl} alt={item.title} className="size-full object-cover transition duration-700 group-hover:scale-[1.035]" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-6 pt-20 text-white"><h3 className="font-medium">{item.title}</h3></div></button>)}</div>
                </section>
            ) : null}
