...
export type ProductionTemplate = { id: string; organizationId: string; name: string; description: string; status: "active" | "disabled"; currentVersion: number; currentPrompt: string; version: number; createdBy: string; createdAt: string; updatedAt: string };
export type ProductionTemplateVersion = { id: string; templateId: string; version: number; prompt: string; createdBy: string; createdAt: string };
...
export type BatchProductionJob = { id: string; organizationId: string; requestId: string; brandId: string; name: string; presetId: string; presetVersion: number; deliverySpec: ProductionDeliverySpec; productIds: string[]; status: BatchProductionStatus; totalItems: number; completedItems: number; failedItems: number; createdBy: string; createdAt: string; updatedAt: string };
export type BatchProductionQualityContext = { brand?: Brand; product: Product; sku?: ProductSKU };
export type BatchProductionItem = { id: string; jobId: string; productId: string; skuId: string; status: BatchProductionStatus; resultStorageKey: string; resultMimeType: string; resultSize: number; qualityContext?: BatchProductionQualityContext; errorMessage: string; reviewStatus: BatchProductionReviewStatus; reviewComment: string; reviewedBy: string; reviewedAt: string; isPrimary: boolean; runNumber: number; attempts: number; createdAt: string; updatedAt: string };
...
export const saveProductionTemplate = (input: Partial<ProductionTemplate> & { prompt: string }) => apiPost<ProductionTemplate>("/api/commerce/production-templates", input);
...
export async function downloadBatchProductionArchive(id: string) {
    ...
    return { blob: await response.blob(), filename: encodedName ? decodeURIComponent(encodedName) : quotedName || "batch-results.zip" };
}
