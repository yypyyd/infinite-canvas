            ...
            <section className="overflow-hidden border-b border-border px-4 pb-8 pt-8 sm:px-8 lg:px-12">
                <div className="mx-auto max-w-[1320px]">
                    <motion.div initial={reducedMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5 }} className="grid items-end gap-7 lg:grid-cols-[.8fr_1.2fr]">
                        <div><p className="text-xs font-medium uppercase tracking-[.2em] text-primary">AI 视觉创作空间</p><h1 className="mt-3 text-4xl font-semibold tracking-[-.07em] sm:text-5xl">今天想创造什么？</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">从一张商品图开始，继续生成、编辑和交付。</p></div>
                        <div className="flex flex-wrap items-center gap-4 lg:justify-end"><Link href="/image?preset=product-main" prefetch={false} className="flex min-h-14 min-w-0 flex-1 items-center gap-3 rounded-xl border border-border bg-card px-4 text-sm text-muted-foreground transition hover:border-primary/30 sm:min-w-[360px] sm:flex-none"><Sparkles className="size-4 shrink-0 text-primary" /><span className="flex-1">描述你想创造的商品画面...</span><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-primary text-primary-foreground"><ArrowUpRight className="size-4" /></span></Link><Link href="/canvas" prefetch={false} className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground transition hover:text-primary"><Layers3 className="size-4" />打开画布</Link></div>
                    </motion.div>
            ...
            <section className="border-y border-border bg-card/70">
                <motion.div initial={reducedMotion ? false : { opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .25 }} transition={{ duration: .65 }} className="mx-auto max-w-[1360px] px-5 py-16 sm:px-8 sm:py-20 lg:px-12">
                    <div className="grid items-center gap-10 lg:grid-cols-[.78fr_1.22fr] lg:gap-14">
                        <div>
                            <div className="mb-5 flex size-9 items-center justify-center rounded-full bg-muted text-primary"><Play className="size-4" /></div>
