	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if decoded.Bounds().Dx() != test.width || decoded.Bounds().Dy() != test.height {
				t.Fatalf("delivery dimensions = %dx%d", decoded.Bounds().Dx(), decoded.Bounds().Dy())
			}
			if format != "jpeg" || result.MimeType != "image/jpeg" || http.DetectContentType(result.Data) != "image/jpeg" {
				t.Fatalf("unexpected JPEG result: format=%s mime=%s", format, result.MimeType)
			}
			if result.Size != int64(len(result.Data)) || result.Size == 0 || result.ResultURL != "" {
				t.Fatalf("unexpected delivery metadata: %#v", result)
			}
		})
	}
}
...
func TestApplyProductionDeliverySpecKeepsOriginalResult(t *testing.T) {
	...
	if !bytes.Equal(result.Data, original.Data) || result.Size != original.Size || result.MimeType != original.MimeType || result.ResultURL != original.ResultURL {
		t.Fatalf("original delivery result changed: %#v", result)
	}
}
