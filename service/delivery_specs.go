...
func prepareProductionDeliveryResult(ctx context.Context, result BatchProductionResult, spec model.ProductionDeliverySpec) (BatchProductionResult, error) {
	...
	if spec.Width <= 0 || spec.Height <= 0 {
		return result, permanentBatchProductionError(model.BatchProductionErrorStorageArchive, errors.New("delivery image dimensions are invalid"))
	}
	if len(result.Data) == 0 {
		...
		client := &http.Client{Transport: transport, Timeout: 15 * time.Minute, CheckRedirect: func(request *http.Request, via []*http.Request) error {
			if len(via) >= 5 || !validHTTPSURL(request.URL.String()) {
				return permanentBatchProductionError(model.BatchProductionErrorStorageArchive, errors.New("batch delivery redirect is not allowed"))
			}
			return nil
		}}
		data, mimeType, err := downloadStandardBatchImage(ctx, client, result.ResultURL, maxUserFileSize)
		if err != nil {
			return result, deliveryBatchProductionError(err)
		}
		...
		if declared := strings.TrimSpace(strings.Split(result.MimeType, ";")[0]); declared != "" && declared != mimeType {
			return result, permanentBatchProductionError(model.BatchProductionErrorStorageArchive, errors.New("batch delivery source MIME does not match executor response"))
		}
...
func applyProductionDeliverySpec(result BatchProductionResult, spec model.ProductionDeliverySpec) (BatchProductionResult, error) {
	...
	if err != nil {
		return result, permanentBatchProductionError(model.BatchProductionErrorStorageArchive, errors.New("generated image cannot be decoded for delivery"))
	}
	...
	case "jpeg":
		...
		if err := imaging.Encode(&output, flattened, imaging.JPEG, imaging.JPEGQuality(spec.Quality)); err != nil {
			return result, permanentBatchProductionError(model.BatchProductionErrorStorageArchive, err)
		}
		result.MimeType = "image/jpeg"
	case "png":
		if err := imaging.Encode(&output, imageResult, imaging.PNG); err != nil {
			return result, permanentBatchProductionError(model.BatchProductionErrorStorageArchive, err)
		}
	...
	if output.Len() == 0 || output.Len() > maxUserFileSize {
		return result, permanentBatchProductionError(model.BatchProductionErrorStorageArchive, errors.New("delivery image is too large"))
	}
