...
export function Minimap({ nodes, viewport, viewportSize, onViewportChange }: { nodes: CanvasNodeData[]; viewport: ViewportTransform; viewportSize: { width: number; height: number }; onViewportChange: (viewport: ViewportTransform) => void }) {
    ...
    return (
        <div className="absolute bottom-40 left-2 z-50 overflow-hidden rounded-lg border shadow-2xl backdrop-blur-sm sm:bottom-24 sm:left-6" style={{ width, height, background: theme.toolbar.panel, borderColor: theme.toolbar.border }}>
            <div
                ref={containerRef}
