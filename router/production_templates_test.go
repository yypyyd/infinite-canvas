...
func TestProductionTemplateHTTPVersionsPreviewAndBatchSnapshot(t *testing.T) {
	...
	if err := database.Create(&sku).Error; err != nil {
		t.Fatal(err)
	}
	...
	if createdResponse.Code != 0 || json.Unmarshal(createdResponse.Data, &template) != nil || template.CurrentVersion != 0 || template.Version != 1 {
		t.Fatalf("create template response: %#v, template=%#v", createdResponse, template)
	}
	...
	if firstPublishResponse.Code != 0 || json.Unmarshal(firstPublishResponse.Data, &firstPublished) != nil || firstPublished.CurrentVersion != 1 || firstPublished.Version != 2 || firstPublished.CurrentPrompt != "模板版本一" {
		t.Fatalf("first publish response: %#v, template=%#v", firstPublishResponse, firstPublished)
	}
	...
	if updatedResponse.Code != 0 || json.Unmarshal(updatedResponse.Data, &updated) != nil || updated.CurrentVersion != 1 || updated.Version != 3 {
		t.Fatalf("update template response: %#v, template=%#v", updatedResponse, updated)
	}
	...
	if secondPublishResponse.Code != 0 || json.Unmarshal(secondPublishResponse.Data, &secondPublished) != nil || secondPublished.CurrentVersion != 2 || secondPublished.Version != 4 || secondPublished.CurrentPrompt != "模板版本二" {
		t.Fatalf("second publish response: %#v, template=%#v", secondPublishResponse, secondPublished)
	}
	if stale := routerTestJSON(t, client, http.MethodPost, baseURL+"/api/commerce/production-templates", model.SaveProductionTemplateInput{ID: template.ID, Name: template.Name, Status: model.ProductionTemplateStatusDraft, Prompt: "过期覆盖", Version: 1}, headers); stale.Code != 1 {
		t.Fatalf("stale template response: %#v", stale)
	}
	...
	if versionsResponse.Code != 0 || json.Unmarshal(versionsResponse.Data, &versions) != nil || len(versions) != 2 || versions[0].Version != 2 || versions[1].Prompt != "模板版本一" {
		t.Fatalf("template versions response: %#v, versions=%#v", versionsResponse, versions)
	}
	...
	if previewResponse.Code != 0 || json.Unmarshal(previewResponse.Data, &preview) != nil {
		t.Fatalf("template preview response: %#v", previewResponse)
	}
	for _, expected := range []string{"模板版本二", "品牌语气：克制专业", "品牌规范：使用暖色柔光", "禁止在画面中出现的文字或概念：夸大宣传", "核心卖点：轻量；耐用", "SKU 属性：颜色=红色", "目标渠道：抖音 商品竖图", "交付画幅：1080×1440"} {
		if !strings.Contains(preview.Prompt, expected) {
			t.Fatalf("preview missing %q: %s", expected, preview.Prompt)
		}
	}
	...
	if batchResponse.Code != 0 || json.Unmarshal(batchResponse.Data, &job) != nil {
		t.Fatalf("template batch response: %#v", batchResponse)
	}
	var savedJob model.BatchProductionJob
	if err := database.First(&savedJob, "id = ?", job.ID).Error; err != nil {
		t.Fatal(err)
	}
	if savedJob.PresetVersion != 1 || savedJob.PresetPrompt != "模板版本一" || savedJob.DeliverySpec.ID != "taobao-main" || savedJob.DeliverySpec.Width != 800 || savedJob.DeliverySpec.Height != 800 {
		t.Fatalf("batch template or delivery spec was not frozen: %#v", savedJob)
	}
	...
	if err := database.Create(&model.OrganizationMember{ID: "member-router-template-reviewer", OrganizationID: tenant.Organization.ID, UserID: reviewer.ID, Role: model.OrganizationRoleReviewer, Version: 1, CreatedAt: "3", UpdatedAt: "3"}).Error; err != nil {
		t.Fatal(err)
	}
	reviewerClient, reviewerURL := loginRouterTestClient(t, reviewer.Username)
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, reviewerURL+"/api/commerce/production-templates/preview", model.PreviewProductionPromptInput{PresetID: template.ID, PresetVersion: 2, ProductID: product.ID}, headers); response.Code != 0 {
		t.Fatalf("reviewer preview response: %#v", response)
	}
	if response := routerTestJSON(t, reviewerClient, http.MethodPost, reviewerURL+"/api/commerce/production-templates", model.SaveProductionTemplateInput{Name: "越权模板", Status: model.ProductionTemplateStatusActive, Prompt: "越权"}, headers); response.Code != 1 {
		t.Fatalf("reviewer template write response: %#v", response)
	}
}
