...
func TestBatchProductionArchiveNamesAreStableAndPathSafe(t *testing.T) {
	...
	if jobName != "秋季-上新.zip" {
		t.Fatalf("archive name = %q", jobName)
	}
	...
	if entry != "商品A/SKU红色/legacy/商品A_SKU红色_主图_1234567890_7af09963a050.png" {
		t.Fatalf("archive entry = %q", entry)
	}
	if strings.HasPrefix(entry, "/") || strings.Contains(entry, "../") || strings.Count(entry, "/") != 3 {
		t.Fatalf("unsafe archive entry = %q", entry)
	}
}
