...
export function filterNodeGenerationInputs(inputs: NodeGenerationInput[], capabilities?: NodeReferenceCapabilities) {
    ...
    return inputs.filter((input) => {
        ...
        if (capabilities.maxMedia !== undefined && capabilities.maxMedia > 0 && mediaCount >= capabilities.maxMedia) return false;
        if (input.type === "image" && capabilities.image && (capabilities.maxImages === undefined || imageCount < capabilities.maxImages)) imageCount += 1;
        else if (input.type === "video" && capabilities.video && (capabilities.maxVideos === undefined || videoCount < capabilities.maxVideos)) videoCount += 1;
        else if (input.type === "audio" && capabilities.audio && (capabilities.maxAudios === undefined || audioCount < capabilities.maxAudios)) audioCount += 1;
        else return false;
        mediaCount += 1;
        return true;
    });
