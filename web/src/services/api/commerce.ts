...
export type BatchProductionJob = {
    ...
    deliverySpec: ProductionDeliverySpec;
    productIds: string[];
...
export type BatchProductionItem = {
    id: string;
    jobId: string;
    productId: string;
    skuId: string;
    ...
    variantIndex: number;
    estimatedCredits: number;
    errorCode: BatchProductionErrorCategory;
    retryable: boolean;
    ...
    resultSize: number;
    qualityContext?: BatchProductionQualityContext;
    errorMessage: string;
    reviewStatus: BatchProductionReviewStatus;
...
export const fetchBatchProductionJob = (id: string) =>
    apiGet<{
        job: BatchProductionJob;
        templateSelections: Array<{ id: string; templateId: string; templateVersion: number; templateType: string; quantity: number; specJson: string; deliverySpec: ProductionDeliverySpec }>;
        progress: { total: number; queued: number; running: number; succeeded: number; failed: number };
    }>(`/api/commerce/batch-jobs/${id}`);
export const createBatchProductionJob = (input: { requestId: string; name: string; brandId?: string; presetId: string; presetVersion: number; deliverySpecId: string; productIds: string[] }) =>
    apiPost<BatchProductionJob>("/api/commerce/batch-jobs", input);
...
export async function downloadBatchProductionArchive(id: string) {
    ...
    try {
        filename = decodeURIComponent((encodedName || quotedName || plainName || filename).trim());
    } catch {
        filename = "batch-results.zip";
    }
